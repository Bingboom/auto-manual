.. raw:: latex

   \HBApplyLang{en}

.. only:: latex

   .. raw:: latex

      \section{PRODUCT OVERVIEW}
      \HBOverviewPanel{FRONT VIEW}{front_product.jpg}{%
      \HBOverviewPair{Main POWER Button}{}{Handle}{}
      \HBOverviewPair{DC 12 V Port}{12 V⎓10 A max.}{LCD}{}
      \HBOverviewPair{DC / USB Power Button}{}{LED Light Button}{}
      \HBOverviewPair{USB-C 30 W Output}{30 W max., 5 V⎓3 A, 9 V⎓3 A, 12 V⎓2.5 A, 15 V⎓2 A, 20 V⎓1.5 A}{LED Light}{}
      \HBOverviewPair{USB-C 140 W Output}{140 W max., 5 V⎓3 A, 9 V⎓3 A, 12 V⎓3 A, 15 V⎓3 A, 20 V⎓5 A, 28 V⎓5 A}{AC Power Button}{}
      \HBOverviewPair{USB-A 18 W Output}{18 W max., 5-6 V⎓3 A, 6-9 V⎓2 A, 9-12 V⎓1.5 A}{AC Output}{230 V~ 50 Hz, 10 A max., 2400 W Rated}
      \HBOverviewFull{Total Output}{2400 W Rated, 4800 W Surge Peak}
      }
      \HBOverviewPanel{RIGHT SIDE VIEW}{right_side_ports.png}{%
      \HBOverviewPair{Handle}{}{AC Input}{220 V-240 V~ 50 Hz, 10 A max.}
      \HBOverviewPair{}{}{DC Input (2×DC8020 Ports)}{PV: 16-60 V⎓12 A max., Double to 21 A / 800 W max. \newline Car: 11-16 V⎓8 A max., Double to 8 A max.}
      }

.. only:: not latex

   PRODUCT OVERVIEW
   ================

   FRONT VIEW
   ----------

   .. image:: _assets/templates/word_template/common_assets/overview/front_product.jpg
      :alt: Front view diagram placeholder.
      :width: 420px

   .. list-table::
      :header-rows: 0
      :widths: 50 50

      * - **Main POWER Button**
        - **Handle**
      * - **DC 12 V Port**

          12 V⎓10 A max.
        - **LCD**
      * - **DC / USB Power Button**
        - **LED Light Button**
      * - **USB-C 30 W Output**

          30 W max., 5 V⎓3 A, 9 V⎓3 A, 12 V⎓2.5 A, 15 V⎓2 A, 20 V⎓1.5 A
        - **LED Light**
      * - **USB-C 140 W Output**

          140 W max., 5 V⎓3 A, 9 V⎓3 A, 12 V⎓3 A, 15 V⎓3 A, 20 V⎓5 A, 28 V⎓5 A
        - **AC Power Button**
      * - **USB-A 18 W Output**

          18 W max., 5-6 V⎓3 A, 6-9 V⎓2 A, 9-12 V⎓1.5 A
        - **AC Output**

          230 V~ 50 Hz, 10 A max., 2400 W Rated

   .. list-table::
      :header-rows: 0
      :widths: 100

      * - **Total Output**

          2400 W Rated, 4800 W Surge Peak

   RIGHT SIDE VIEW
   ---------------

   .. image:: _assets/templates/word_template/common_assets/overview/right_side_ports.png
      :alt: Right side view diagram placeholder.
      :width: 420px

   .. list-table::
      :header-rows: 0
      :widths: 50 50

      * - **Handle**
        - **AC Input**

          220 V-240 V~ 50 Hz, 10 A max.
      * -
        - **DC Input (2×DC8020 Ports)**

          PV: 16-60 V⎓12 A max., Double to 21 A / 800 W max.

          Car: 11-16 V⎓8 A max., Double to 8 A max.
