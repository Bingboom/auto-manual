НАЛАШТУВАННЯ ДОДАТКА
====================

**1. Завантажте додаток і увійдіть**

.. image:: templates/word_template/common_assets/app/download.png
   :alt: Заглушка QR-коду для завантаження додатка та магазину застосунків.
   :width: 320px

Знайдіть "Jackery" у Google Play або App Store, щоб установити додаток. Після цього ви можете зареєструватися та увійти.
Або відскануйте QR-код нижче, щоб завантажити та встановити додаток.

**2. Додайте пристрій**

.. |add_device_icon| image:: templates/word_template/common_assets/app/add_device_plus.svg
   :alt: Значок додавання пристрою.
   :width: 14px

2.1 Натисніть кнопку |add_device_icon|, щоб додати свій пристрій.

2.2 Натисніть |MAIN_POWER_BUTTON_LABEL| на пристрої, щоб увімкнути його; значки Wi-Fi та Bluetooth на пристрої блимають, що означає перехід пристрою в режим налаштування мережі, торкніться кнопки "**Значок блимає**" і дозвольте додатку підключитися до найближчих пристроїв та надати дозвіл Bluetooth.

.. image:: templates/word_template/common_assets/app/add_device.png
   :alt: Заглушка кроку додавання пристрою в додатку.
   :width: 320px


| |MAIN_POWER_BUTTON_LABEL|
| |AC_POWER_BUTTON_LABEL|
| |DC_USB_POWER_BUTTON_LABEL|
|

| 2.3 Після натискання на значок знайденого пристрою додаток автоматично підключить пристрій через Bluetooth.

.. list-table::
   :header-rows: 0
   :widths: 12 88

   * - **ПРИМІТКА**
     - Якщо під час процесу прив'язування з'являється повідомлення "**пристрій уже прив'язано**", для підключення можна скористатися одним із двох способів:

       - Власник пристрою поділиться цим пристроєм з іншими користувачами через додаток.
       - Натисніть і утримуйте |MAIN_POWER_BUTTON_LABEL_LOWER| + |DC_USB_POWER_BUTTON_LABEL_LOWER| протягом 3 секунд, щоб скинути Wi-Fi та Bluetooth пристрою, а потім знову прив'яжіть пристрій.

| 2.4 Після успішного підключення пристрою введіть пароль Wi-Fi та натисніть кнопку **ОК**.

.. list-table::
   :header-rows: 0
   :widths: 12 88

   * - **ПРИМІТКА**
     -
       - Будь ласка, вибирайте мережу Wi-Fi у діапазоні 2,4 ГГц. Пристрій не підтримує мережі Wi-Fi у діапазоні 5 ГГц.

| 2.5 Після успішного додавання пристрою до додатка значок Wi-Fi на пристрої буде постійно увімкнений.

.. image:: templates/word_template/common_assets/app/connect_result.png
   :alt: Заглушка екрана результату налаштування додатка.
   :width: 360px

| Наведені вище знімки екрана наведено лише для довідки.


.. list-table::
   :header-rows: 0
   :widths: 12 88

   * - **УВАГА**
     - Додаток Jackery може одночасно підключатися лише до однієї електростанції через Bluetooth. Повернення до списку пристроїв автоматично відключає Bluetooth. Щоб знову підключитися автоматично, торкніться електростанції у списку ще раз.


**3. Відв'яжіть пристрій**

Торкніться значка **Налаштування** у правому верхньому куті головного інтерфейсу пристрою, щоб перейти на сторінку налаштувань, а потім торкніться кнопки **Відв'язати** внизу сторінки, щоб відв'язати пристрій.

**4. Примітки**

**4.1 Щоб увімкнути Wi-Fi та Bluetooth**

- Wi-Fi та Bluetooth автоматично вмикаються після увімкнення пристрою, а значки Wi-Fi та Bluetooth на екрані загоряються.
- Одночасно натисніть і утримуйте |DC_USB_POWER_BUTTON_LABEL_LOWER| + |AC_POWER_BUTTON_LABEL_LOWER|, доки значки Wi-Fi та Bluetooth на екрані не загоряться.

**4.2 Щоб вимкнути Wi-Fi та Bluetooth**

| Одночасно натисніть і утримуйте |DC_USB_POWER_BUTTON_LABEL_LOWER| + |AC_POWER_BUTTON_LABEL_LOWER|, доки значки Wi-Fi та Bluetooth на екрані не згаснуть.


**4.3 Щоб скинути Wi-Fi та Bluetooth**

Одночасно натисніть і утримуйте |MAIN_POWER_BUTTON_LABEL_LOWER| + |DC_USB_POWER_BUTTON_LABEL_LOWER| протягом 3 секунд, щоб скинути Wi-Fi та Bluetooth до заводських налаштувань. Обліковий запис підключеного додатка буде відв'язано.

