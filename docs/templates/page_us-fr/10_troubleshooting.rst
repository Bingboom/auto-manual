DÉPANNAGE
=========

Si l'un des codes d'erreur suivants apparaît, suivez les actions correctives indiquées pour résoudre le problème.
Si l'erreur persiste, veuillez contacter le service clientèle de Jackery.

.. list-table::
   :header-rows: 1
   :widths: 14 86

   * - Code d'erreur
     - Mesures correctives
   * - F0
     - Redémarrez le produit.
   * - F1
     - Redémarrez le produit.
   * - F2
     - Redémarrez le produit.
   * - F3
     - Redémarrez le produit.
   * - F4
     - Connectez le produit à des charges afin de décharger sa batterie jusqu'à ce que l'erreur disparaisse.
   * - F5
     - Chargez le produit via des panneaux solaires ou une prise murale CA jusqu'à ce que l'erreur disparaisse.
   * - F6
     - 1. Attendez que le réseau se normalise avant de charger le produit via une prise murale CA.
       2. Vérifiez si les entrées et sorties d'air sont obstruées ; assurez un espace libre de 0,66 pied (20 cm) de chaque côté du produit.
       3. Placez le produit dans un endroit qui n'est pas exposé à la lumière directe du soleil ni à des températures ambiantes élevées.
       4. Déconnectez toutes les charges du produit. Laissez le produit inactif et attendez que l'erreur disparaisse.
       5. Redémarrez le produit.
   * - F7
     - 1. Retirez toutes les entrées CC du produit.
       2. Vérifiez la tension en circuit ouvert (Voc) des panneaux solaires connectés. Le produit accepte une tension d'entrée CC maximale de 60 V.
       3. Redémarrez le produit et laissez-le inactif. Attendez que l'erreur disparaisse.
   * - F8
     - Contactez le service clientèle de Jackery.
   * - F9
     - Retirez la charge connectée aux ports USB du produit. Attendez que l'erreur disparaisse.
   * - FE
     - Contactez le service clientèle de Jackery.
