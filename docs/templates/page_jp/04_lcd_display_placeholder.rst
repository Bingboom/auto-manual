液晶画面
========

.. image:: templates/word_template/common_assets/lcd/lcd_map.png
   :alt: LCD icon map.
   :width: 420px

.. list-table::
   :header-rows: 1
   :widths: 10 30 60

   * - ID
     - 表示項目
     - 説明
   * - 1
     - Wi-Fi
     - **オン**：Wi-Fi接続済。

       **点滅**：Wi-Fiに接続する準備ができました。

       **オフ**：Wi-Fi接続解除済。
   * - 2
     - Bluetooth
     - **オン**：Bluetooth接続済。

       **点滅**：Bluetoothに接続する準備ができました。

       **オフ**：Bluetooth接続解除済。
   * - 3
     - 静音充電モード
     - **オン**：充電中のノイズが大幅に低減され、充電電力が低下し、充電速度が低減します。

       **オフ**：静音充電モードが無効になっています。

       （Jackeryアプリで有効/無効にできます。電源オフ時も設定は保持されます。）
   * - 4
     - 計画充電モード
     - |PRODUCT_NAME| の充電時間をカスタマイズすることで、電気料金のピーク時間とオフピーク時間に基づいた充電プランを選択。これにより月々の電気料金を削減することができます。

       （Jackeryアプリで設定してください。電源オフ時も設定は保持されます。）
   * - 5
     - 自家発電モード
     - **点灯時**：自家発電モードがオンになっており、太陽光エネルギーの利用を最大化し、電力会社からの電力消費を削減して電気代を節約します。

       **消灯時**：自家発電モードがオフ、または未設定の状態です。

       （Jackeryアプリで設定してください。電源オフ時も設定は保持されます。）
   * - 6
     - UPS機能
     - **点灯時**：製品はバイパスモードで動作しています。ACポートに接続された負荷は、ポータブル電源ではなく商用電源から給電されます。

       商用電源が突然停電した場合、製品は10ミリ秒以内に自動でバッテリー給電へ切り替わります。

       **消灯時**：製品はバイパスモードではありません。ACポートに接続された負荷は、ポータブル電源の内蔵バッテリーから給電されます。
   * - 7
     - 交流電源による出力マーク（正弦波)
     - AC出力（純正弦波）がオンになっています。
   * - 8
     - 出力電圧と周波数
     - 出力される電圧と周波数を表示します。
   * - 9
     - 入力電力表示
     - 入力電力をワットで表示します。
   * - 10
     - 充電残り時間
     - 充電残り時間を表示します。
   * - 11
     - AC入力電力表示
     - 製品は、系統電力を使用し、AC入力を介して充電されます。
   * - 12
     - シガーソケット入力電力表示
     - 製品は、DC 12V（カーチャージ）を使用し、8020入力ポートを介して充電されています。
   * - 13
     - ソーラー(グリーンエネルギー)充電接続
     - 製品は、ソーラーパネルを使用し、8020入力ポートを介して充電されています。
   * - 14
     - バッテリー節約モード

       Jackeryアプリで有効/無効にできます
     - バッテリー節約モードが有効になると、充電および放電の制限が適用され、バッテリーの寿命を延ばすことが可能です。

       （Jackeryアプリで設定してください。電源オフ時も設定は保持されます。）

       この機能が有効な場合、本製品はSOCを較正するために、時折フル充放電サイクルを実行します。
   * - 15
     - 充電速度制限
     - 点灯時：アプリ側で充電速度制限が有効になっています。

       消灯時：アプリ側で充電速度制限が無効になっています。

       （Jackeryアプリで設定してください。電源オフ時も設定は保持されます。）
   * - 16
     - バッテリーアイコン
     - 製品の充電中は、バッテリーのパーセンテージを囲むオレンジ色の円が順番に点灯します。他のデバイスの充電時には、オレンジ色の円は点灯したままになります。
   * - 17
     - バッテリーレベルパーセントタグ
     - バッテリー残量を表示します。
   * - 18
     - バッテリー残量警告灯
     - オン：バッテリー残量が20％未満です。

       点滅：バッテリー残量が5％未満です。

       オフ：製品は充電中です。
   * - 19
     - 省エネモード
     - ACまたはDC/USB出力ボタンを押して出力をオンにしたとき：

       **アイコン表示**：省エネモードがオンになっています。

       **アイコン消える**：省エネモードがオフになっています。
   * - 20
     - 高温インジケーター
     - 高温保護が作動しました。製品の温度が通常の動作範囲に戻るまで、製品の機能が停止する場合があります。
   * -
     - 低温インジケーター
     - 低温保護が作動しました。製品の温度が通常の動作範囲に戻るまで、製品の機能が停止する場合があります。
   * - 21
     - エラーコード
     - 製品エラーが発生しました。詳細については、トラブルシューティングのセクションを参照してください。
   * - 22
     - 消費電力
     - 出力電力をワットで表示します。
   * - 23
     - バッテリー使用可能時間
     - 接続した機器の使用可能な時間を表示します。
