...
本製品は、電気系統（家庭内の電源など）が突然停止した場合、自動的にバッテリー給電モードに切り替わるUPS（無停電電源装置）機能を備えています。
切替時間は約10ミリ秒（0.010秒）以内です。

ただし、本機能は0msでの完全な無停止給電には対応しておりません。
そのため、以下のような高精度な連続電源供給を必要とする機器への接続はお控えください。
...
複数の機器をUPSモードで同時に接続する場合は、合計負荷電力がバイパス時の最大出力（1500W）を超えないようにしてください。超えると、機器が過負荷保護により停止するおそれがあります。
