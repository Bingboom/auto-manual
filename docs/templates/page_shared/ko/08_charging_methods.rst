...
본 제품은 두 개의 |DC_INPUT_CONNECTOR| 입력 포트를 갖추고 있으며 Jackery 태양광 패널과 호환됩니다.

.. image:: asset:charging/solar_direct
   :alt: Solar charging connection diagram placeholder 1.
   :width: 360px

하나의 |DC_INPUT_CONNECTOR| 입력 포트에 두 개의 태양광 패널을 동시에 연결해야 하는 경우, 아래 그림을 참조하여 태양광 패널 커넥터(별도 판매, 기본 구성품 아님)를 통해 충전하십시오.
...
.. list-table::
   ...
   * - **주의**
     - 하나의 |DC_INPUT_CONNECTOR| 입력 포트에는 최대 두 개의 태양광 패널만 연결할 수 있습니다.
...
.. list-table::
   ...
   * - **주의**
     - 두 DC 입력 포트의 입력 전압이 동일한지 확인하십시오. 이를 준수하지 않을 경우 제품이 손상될 수 있습니다. 예를 들어:
       ...
       - 두 개의 |DC_INPUT_CONNECTOR| 입력 포트에 태양광 패널을 연결할 때에는 동일한 모델의 Jackery 태양광 패널을 동일한 개수로 사용하십시오.
       - 차량용 충전기와 태양광 패널을 동시에 사용하여 제품을 충전하지 마십시오. 이렇게 할 경우 차량 퓨즈가 끊어지거나 충전에 실패할 수 있습니다.

제품 충전에는 Jackery 태양광 패널 사용을 권장합니다. 사용 중인 태양광 패널의 개방 회로 전압(V\ :sub:`oc`)이 본 제품의 DC 입력 범위(|PV_INPUT_RANGE|) 내에 있는지 확인하십시오. 타사 태양광 패널 사용으로 인해 발생한 손상 또는 손실에 대해 Jackery는 책임을 지지 않습니다.
