...
% ---- spec table wrapper ----
% longtable instead of HBSharedDataTable (tcolorbox + tabularx): the box
% cannot break across pages and "clip upper" silently clips overflowing
% rows (reports/typography_gap/2026-07-03 #5). The left-column tint that
% the frame overlay used to paint is reproduced with \columncolor.
% Trade-off: rounded outer corner dropped. Rows are emitted by
% renderers_spec.py as "label & value \\" with \hline separators.
\newenvironment{spectable}{%
  ...
  % \RaggedRight (ragged2e) instead of \raggedright: allows hyphenation and
  % balanced breaks in the value column, avoiding one-word orphan lines
  % like "Surge Peak" (typography_gap #7).
  \begin{longtable}{|
    ...
    >{\RaggedRight\arraybackslash}m{\dimexpr\linewidth
      -\csname HBcomp_spec_table_left_ratio\endcsname\linewidth
