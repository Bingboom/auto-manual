...
\expandafter\providecommand\csname HBcomp_lcd_grey_width_ratio\endcsname{0.41}
...
\providecommand{\HBTypeLcdLabel}[1]{%
  {\leftskip=\csname HBcomp_table_text_indent\endcsname\relax
   \HBFontSemiBold
...
% The renderer emits bounded segments (rows 1-7, then continuation groups),
% allowing every LCD segment to reuse the same complete rounded shell as all
% other data tables without clipping rows at a page boundary.
\NewEnviron{HBLcdIconTable}{%
  \par\noindent
  \begingroup
  \colorlet{HBSharedTableLeftBg}{BgK05}%
  \colorlet{HBSharedTableFrameColor}{LineK40}%
  \begin{HBSharedDataTable}
    {\csname HBcomp_lcd_grey_width_ratio\endcsname}
    {\csname HBcomp_lcd_table_tabcolsep\endcsname}%
