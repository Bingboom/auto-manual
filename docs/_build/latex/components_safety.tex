...
\begin{safetywarningbox}
  ...
  \begin{tabularx}{\textwidth}{@{}p{\csname HBwarn_col_icon\endcsname} p{\csname HBwarn_col_label\endcsname} X@{}}
    ...
    \raggedright{\bfseries\fontsize{10pt}{12pt}\selectfont #1} &
    \raggedright{\bfseries\fontsize{\csname HBwarn_text_size\endcsname}{7.8pt}\selectfont\MakeUppercase{#2}}
  \end{tabularx}
...
% brand_title_width is ratio (e.g., 0.86)
% --------------------------------------------------
\providecommand{\hbsectiontitle}[1]{#1}

\renewcommand{\hbsectiontitle}[1]{%
  \par\noindent
  \hspace*{-\leftskip}% 关键：抵消当前环境左缩进（Sphinx 标题常有）
  \begin{tcolorbox}[
    colback=BrandDark,
    colframe=BrandDark,
    ...
    width=\textwidth % 关键：用版心宽，不用 linewidth
  ]

  {\color{white}\bfseries\fontsize{9pt}{10.8pt}\selectfont\MakeUppercase{#1}}
  \end{tcolorbox}
  \vspace{\csname HBbrand_title_vspace_after\endcsname}
}
