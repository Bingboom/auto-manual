% ==================================================
% components_base.tex
% 通用组件 / 通用排版规则（基础层, layered params）
% Parameter access: \csname HB<key>\endcsname
% ==================================================

% ------------------------------
% List base (enumitem)
% ------------------------------
\setlist[itemize]{
  leftmargin=\csname HBcomp_list_leftmargin\endcsname,
  labelsep=\csname HBcomp_list_labelsep\endcsname,
  itemsep=\csname HBcomp_list_itemsep\endcsname,
  topsep=\csname HBcomp_list_topsep\endcsname,
  parsep=\csname HBcomp_list_parsep\endcsname,
  partopsep=\csname HBcomp_list_partopsep\endcsname,
  label=\raisebox{\csname HBcomp_list_bullet_raise\endcsname}{\csname HBcomp_list_bullet_symbol\endcsname}
}

% ------------------------------
% Table base
% ------------------------------
\arrayrulewidth=\csname HBcomp_table_inner_rule\endcsname
\setlength{\tabcolsep}{\csname HBcomp_table_tabcolsep\endcsname}

\newtcolorbox{tableframe}{
  enhanced,
  colback=white,
  colframe=LineK40,
  arc=\csname HBcomp_table_outer_arc\endcsname,
  boxrule=\csname HBcomp_table_outer_rule\endcsname,
  left=0mm,right=0mm,top=0mm,bottom=0mm,
}

% ------------------------------
% Note box
% ------------------------------
\newtcolorbox{notebox}{
  enhanced,
  colback=BgK05,
  colframe=BgK05,
  arc=\csname HBcomp_note_arc\endcsname,
  boxrule=0pt,
  left=\csname HBcomp_note_pad_lr\endcsname,
  right=\csname HBcomp_note_pad_lr\endcsname,
  top=\csname HBcomp_note_pad_tb\endcsname,
  bottom=\csname HBcomp_note_pad_tb\endcsname,
}

% ------------------------------
% Helper: lock natural-wrap line spacing in boxes/tables
% - 强制自然换行行距严格跟随 leading（解决 tabular/array 的 lineskip fallback）
%
% Usage:
%   \HBtightlines{<leading>}{<content>}
% Example:
%   \HBtightlines{\csname HBtype_warning_text_font_leading\endcsname}{#1}
% ------------------------------
\providecommand{\HBtightlines}[2]{%
  \begingroup
    \setlength{\baselineskip}{#1}%
    \setlength{\lineskip}{0pt}%
    \setlength{\lineskiplimit}{0pt}%
    \setlength{\parskip}{0pt}%
    \setlength{\parindent}{0pt}%
    \strut #2\strut
  \endgroup
}

% ------------------------------
% Rubric style (subsubsection*)
% - spacing: COMPONENT
% - font: TYPE SYSTEM
% ------------------------------
\makeatletter
\renewcommand\subsubsection{\@startsection{subsubsection}{3}{0pt}%
  {\csname HBcomp_rubric_before\endcsname}%
  {\csname HBcomp_rubric_after\endcsname}%
  {\bfseries
   \fontsize{\csname HBtype_rubric_font_size\endcsname}
            {\csname HBtype_rubric_font_leading\endcsname}
   \color{\csname HBtype_rubric_color\endcsname}
   \selectfont\color{black}}}
\makeatother


% ------------------------------
% Tight lines helper (alignment + locked leading)
% ------------------------------

\renewcommand{\HBtightlines}[2]{%
  \begingroup
    \setlength{\baselineskip}{#1}%
    \setlength{\lineskip}{0pt}%
    \setlength{\lineskiplimit}{0pt}%
    \parindent=0pt
    \strut #2\strut
  \endgroup
}








