...
- structured data changes: Feishu phase2 source tables are the authoring surface; [`data/phase2/`](data/phase2) is the local snapshot root used by build, review, and publish flows
  `Localized_Copy.csv` is the shared short-copy table for reusable page chrome and labels such as LCD / Symbols page titles, table headers, LCD image alt text, and Product overview part labels. Keep long instructional prose in RST templates, model/spec values in `Spec_Master.csv`, Symbols row content in `symbols_blocks.csv`, and LCD description variables in `Variable_Defaults.csv` plus `Variable_Lang_Overrides.csv`. Symbols image alt text is derived from `symbol_key` or signal-row labels instead of duplicated as copy rows. LCD status-word bolding is driven by `Status_Words.csv`, a local snapshot of Translation Memory rows marked `是否为 status word=Y`, so status words are not duplicated in `Localized_Copy.csv` or the LCD icon table.
