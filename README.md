...
- for `build.queue_by_document_key` configs, `Build Draft Package` rows with a non-empty `Lang` are split by normalized language instead of being merged only by `Document_Key`; `Lang=br` / `pt-br` maps to `pt-BR`, and that selected language is forwarded through `check`, `word`, `md`, `publish`, bundle materialization, and output-path resolution. This lets one `JE-1500D_pt-BR` review key produce separate `US/en` and `pt-BR/br` draft packages.
...
- merged review-init and build-queue rows should use `Build_family = us-merged` / `eu-merged` and may leave `Lang` blank; paired source/target review families such as `pt-br` may use one `Document_Key` for Start Review and separate Draft rows for `Lang=en` and `Lang=br` / `pt-BR`; single-language rows should use the matching single-language family such as `us-en` / `eu-en` / `us-fr` / `us-es`
- config policy for `build.queue_by_document_key`: enable it for merged whole-book families that intentionally represent one shared manual across languages, such as today's `us-merged`, `eu-merged`, and future `cn-merged`, and for paired source/target review families such as `pt-br` where Draft rows are intentionally split by `Lang`; keep it disabled for single-language families such as `us-en`, `eu-en`, `us-fr`, `us-es`, `jp-ja`, `cn-zh`, or future `eu-de` / `eu-fr`, which should continue to route one row per `record_id`
