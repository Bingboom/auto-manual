...
- `symbols_blocks.order` must be filled with unique values such as `1, 2, 3...`; `table_row` entries are sorted and split evenly into two columns, `signal_row` entries must provide `symbol_key` values `WARNING`, `CAUTION`, `NOTE`, and `TIPS`, and the signal-row meanings must stay in `symbols_blocks`; symbols page header and signal-label copy belongs in `symbols_page_copy` with `copy_key` as the first column and language columns such as `en`, `fr`, and `es`; symbols page titles and alt text stay in `page_copy`
