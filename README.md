...
- for phase2 tables that carry an `Is_Latest` column, `sync-data` now writes only rows where `Is_Latest=TRUE`, even if the upstream Feishu view is broader than expected
...
- `python build.py listen-sync-data --config config.yaml --data-root data/phase2` is the local auto-sync listener for the phase2 source tables: it registers the current base for docs events through the local `lark-cli` user identity, listens on the app bot event stream, watches `spec_titles / spec_footnotes / spec_notes / symbols_blocks / spec_master`, and triggers a coalesced `sync-data` run when any of those source tables changes
- after that local source-table sync finishes, the listener now also runs `build.py sync-review --sync-scope params --data-root data/phase2` for every review bundle that already exists in the current checkout, so local review RST stays aligned with the latest synced phase2 data
- `python build.py listen-phase2-events --config config.yaml --data-root data/phase2` is the recommended local always-on listener: it keeps one Feishu event stream for both source-table auto-sync and `Document_link` immediate-build triggers, which matters because the current `lark-cli event +subscribe` only allows one subscriber per app
- all three local listener entrypoints now share one single-instance lock for that Feishu event stream; if `listen-phase2-events`, `listen-sync-data`, or `listen-build-queue` is already running, a second launch exits cleanly instead of starting another competing long-connection consumer
- local phase2 listeners now fail fast unless the app/bot itself can register the target base for `drive.file.bitable_record_changed_v1`; `lark-cli event +subscribe` only supports bot today, so link-sharing alone is not enough if the bot still cannot subscribe to that bitable document
- local immediate-build listeners now do a startup catch-up scan for rows that are already `镜像...=Y + 是否立即构建=true`, and they also reconcile the current queue state on any `Document_link` add/edit event instead of relying only on checkbox-diff payload details
- keep the identity boundary explicit:
  - local manual commands such as `sync-data`, `sync-review`, `process-build-queue`, `check`, `word`, and `publish` still default to your local `lark-cli auth login` user identity unless you explicitly override `FEISHU_PHASE2_IDENTITY`
  - remote GitHub Actions workers use `FEISHU_PHASE2_IDENTITY=bot`
  - local long-connection listeners are the special case: their record fetch/build work still runs locally, but the live `event +subscribe` transport itself is bot-scoped because the current `lark-cli` listener path only supports bot
  - in other words, “local machine” and “bot identity” are not the same concept; only the event-stream transport is bot-bound, while ordinary local build/sync commands remain user-driven by default
...
- `python build.py process-build-queue --config config.yaml --data-root data/phase2` consumes the `sync.phase2.document_link` task table, first refreshes `data/phase2` with one fresh `sync-data` pull, then writes `开始构建时间` as soon as a pending row starts, builds pending `Document_Key + Lang` rows where `是否触发文档构建 = Y`, uploads the generated Word file to Feishu Drive, then tries to move that uploaded file into the current wiki knowledge-base container before writing the local Word path back to `Document directory`, the final link back to `Document link`, a timestamped status string to `构建结果`, and the trigger back to `已构建` on success
- `Doc_phase` on `Document_link` is optional: `Draft` tells the queue to build from the current review tree with `check --source review + word --source review`, `Publish` tells it to run `build.py publish` for that target, and a blank `Doc_phase` keeps the legacy `check + word` path
- once a target is in review, the review-based build entrypoints now auto-run `sync-review --sync-scope params` before `check/word/html/pdf/all/publish` whenever that target already has a review bundle, so Draft and Publish builds refresh review-side parameter data before exporting documents
- if the only requirement is "build with the latest Feishu phase2 data", the queue path is enough; you do not need a local Feishu long-connection listener because `process-build-queue` now refreshes `data/phase2` before exporting
- `process-build-queue` now also accepts queue scoping: use `--doc-phase draft|publish` to consume only one stage, and `--record-id <Document_link record_id>` when one workflow should process just one queue row
...
- [`scripts/listen_sync_data.ps1`](scripts/listen_sync_data.ps1) is the Windows-friendly wrapper for source-table auto-sync; it restores the local Node/npm path plus the saved `FEISHU_PHASE2_*` user env vars, starts `listen-sync-data`, and writes logs into [`.tmp/sync-data-listener/`](.tmp/sync-data-listener)
- [`scripts/listen_phase2_events.ps1`](scripts/listen_phase2_events.ps1) is the Windows-friendly wrapper for the unified listener; it restores the local Node/npm path plus the saved `FEISHU_PHASE2_*` user env vars, starts `listen-phase2-events`, and writes logs into [`.tmp/phase2-event-listener/`](.tmp/phase2-event-listener)
- `python build.py listen-build-queue --config config.yaml --data-root data/phase2` starts the push-based queue listener: it registers the current `Document_link` base for docs events through the local `lark-cli` user identity, waits on the app bot long connection, and triggers `process-build-queue` immediately when the `是否立即构建` checkbox is checked on a `Document_link` row
...
- [`.github/workflows/feishu-build-queue.yml`](.github/workflows/feishu-build-queue.yml) is the remote GitHub Actions worker: after merge to the default branch and after repo secrets are configured, it runs every 5 minutes plus `workflow_dispatch`, uses `FEISHU_PHASE2_IDENTITY=bot`, and lets `process-build-queue` refresh `data/phase2` itself before consuming the `Document_link` queue, without relying on any local machine
- [`.github/workflows/feishu-build-queue.yml`](.github/workflows/feishu-build-queue.yml) is now the Publish-stage queue worker: it consumes only `Doc_phase=Publish` rows from `main`
- [`.github/workflows/feishu-draft-build-queue.yml`](.github/workflows/feishu-draft-build-queue.yml) is the Draft-stage queue worker: dispatch it against a PR branch ref and it consumes only `Doc_phase=Draft` rows, so review links are generated from the PR branch content instead of `main`
- if you split queue ownership by stage, the intended contract is:
  - `Doc_phase=Draft`: Feishu automation dispatches `feishu-draft-build-queue.yml` with the PR head branch as the GitHub `ref`, plus `queue_record_id` when one row should be rebuilt
  - `Doc_phase=Publish`: `main` owns the row through `feishu-build-queue.yml`, either by scheduled poll or by a `workflow_dispatch` against `main`
- set the repository variable `FEISHU_BUILD_QUEUE_PAUSED=true` when you want to pause that remote queue temporarily without deleting the workflow; clear it or set it back to `false` when the poller should resume
- [`.github/workflows/phase2-word-preview.yml`](.github/workflows/phase2-word-preview.yml) is the PR-side phase2 validation worker: it syncs `data/phase2` once from Feishu, runs `check + word` for the explicit validation target matrix using the pull-request branch code, and uploads the generated `.docx` files as GitHub Actions artifacts without consuming the `Document_link` queue
...
- when that wiki move step fails, the queue now still preserves the latest uploaded Drive URL in `Document link`, keeps the latest local DOCX path in `Document directory`, clears `是否立即构建` to avoid endless local retry loops, and records the permission/move failure in `构建结果` instead of leaving a stale old document link in place
...
- for syncing changes back to your local machine, prefer `listen-sync-data` over a Feishu cloud workflow; the cloud workflow cannot directly call your local computer unless you separately expose a webhook/service from your machine
- if you need both local auto-sync and local immediate-build listening on the same machine, use `listen-phase2-events` instead of running `listen-sync-data` and `listen-build-queue` at the same time; the current `lark-cli` only allows one `event +subscribe` consumer per app
