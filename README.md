...
- once a review bundle exists, `check`, `html`, `word`, `pdf`, `myst`, and `publish` automatically prepare the runtime bundle and run the same parameter sync before export
...
- `python build.py process-build-queue --config config.us.yaml` consumes the `sync.phase2.document_link` task table, writes `开始构建时间` as soon as a pending row starts, resolves the build config from `Build_family` first and `Lang` second, groups only the rows whose resolved config enables `build.queue_by_document_key`, keeps Build Draft Package rows on DOCX upload, upgrades Publish rows to `check -> diff-report -> word -> pdf`, then appends `myst --source review --no-clean` for the Read the Docs / Sphinx Markdown artifact, uploads the Publish PDF to the primary Feishu/wiki destination, can then sync that same PDF to DingTalk as an optional mirror, writes the local DOCX release path back to `Document directory`, writes the canonical PDF URL back to `Document link`, writes the created cloud-doc URL back to `飞书云文档` when that field exists, optionally writes the DingTalk node URL to `Document link_dd`, adds sync and `feishu_doc=ok|failed|skipped` notes into `构建结果`, writes the refresh result back to `data_sync`, and flips the trigger back to `已构建`
...
- when the queue row includes `Version`, Build Draft Package DOCX names use `manual_<model>_<region>_<lang>_<Version>.docx`, while Publish queue release artifacts use `manual_<model>_<region>_<lang>_publish_<Version>.*` for DOCX, PDF, and MyST Markdown
...
python3 build.py myst --config config.us.yaml --model JE-1000F --region US
```
...
- `python build.py myst --config ... --model ... --region ...` now creates a parallel MyST-compatible Markdown artifact under [`docs/_build/<model>/<region>/myst/`](docs/_build) or the language-scoped target root; `build.py all` intentionally still means `html,word,pdf`
