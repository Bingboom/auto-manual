...
- `Manual Validation` now also runs a low-noise maintainability guardrail check so known orchestration and validation hotspots do not silently regrow past their agreed size ceilings, and so visible manual copy does not move back into Python or YAML config
...
- single-language US review bundles from `config.us-en.yaml` still live under `docs/_review/<model>/US/en/`, and Brazil Portuguese review bundles from `config.pt-br.yaml` live under `docs/_review/<model>/pt-BR/pt-BR/`; the merged US queue/review flow driven by `config.us.yaml` now uses `docs/_review/<model>/US/` and exports one combined `en + fr + es` Word under `docs/_build/<model>/US/word/`
...
- `config.us-en.yaml`, `config.us-es.yaml`, `config.us-fr.yaml`, and `config.pt-br.yaml` now inherit shared single-language defaults from [`config-bases/us-single-language-base.yaml`](config-bases/us-single-language-base.yaml); US EN/ES/FR page stacks live in [`docs/manifests/manual_us-single-*.yaml`](docs/manifests), and the Brazil Portuguese stack lives in [`docs/manifests/manual_pt-br.yaml`](docs/manifests/manual_pt-br.yaml)
- `config.eu.yaml` now models the live `EU` row from the region-config Base as `Build_family = eu-merged`, reads `JE-1000F / EU` spec rows from the shared split spec source tables, and routes blank-`Lang` queue rows to the merged EU family instead of misclassifying them as English-only
...
- `sync-data` uses the local `lark-cli` login and `sync.phase2.*` config/env bindings to write normalized CSV snapshots into [`data/phase2/`](data/phase2), using the CLI's `base` record listing flow under the hood; `sync-data --table spec_master` now reads `规格参数明细` and `页面占位参数` directly when `sync.phase2.spec_master_sources` is configured
- `spec-master-rebuild` merges the two Feishu-maintained spec source tables into the existing `Spec_Master.csv` read-model shape, resolves linked-record footnote refs to stable `Footnote_id` values, and leaves `--write-back` as a compatibility bridge for the legacy total table
...
- `build.py validate` fails early when a configured phase2 table has no base-token binding or table id binding, and when a page manifest references a language outside `build.languages`
- `lcd_icons`, `troubleshooting`, `symbols_blocks`, `symbols_page_copy`, `signal_words`, `page_copy`, `variable_defaults`, and `variable_lang_overrides` are first-class phase2 sync tables; LCD icon `figure` attachments sync into `data/phase2/_attachments/lcd_icons/`, symbols `Figure` attachments sync into `data/phase2/_attachments/symbols/`, symbols signal-word rows can use `block_type=signal_row`, symbols header and signal-label copy lives in `symbols_page_copy`, shared reusable signal-word labels live in `signal_words`, and LCD variable placeholders resolve through `Variable_Defaults.csv` plus exact language rows in `Variable_Lang_Overrides.csv`
- `page_copy.csv` is the synced snapshot for shared page copy such as titles, table headers, prompts, alt text, default placeholder copy, and output titles; the fixed columns are `page_id`, `lang`, `copy_key`, `text`, `enabled`, and `order`, and renderers that require a key must fail fast instead of falling back to Python literals
- `troubleshooting` renders rows from `data/phase2/troubleshooting_blocks.csv`; keep rows in the TROUBLESHOOTING Base table with `Region`, `Model`, `Is_latest`, `error_code`, and `corrective_measures_*` columns, while page title, intro, and headers belong in RST or `page_copy`; layout-only settings such as table widths and header-row behavior stay renderer-owned
...
- `symbols_blocks.order` must be filled with unique values such as `1, 2, 3...`; `table_row` entries are sorted and split evenly into two columns, `signal_row` entries must provide `symbol_key` values `WARNING`, `CAUTION`, `NOTE`, and `TIPS`, and the signal-row meanings must stay in `symbols_blocks`; symbols page header and signal-label copy belongs in `symbols_page_copy` with `copy_key` as the first column and language columns such as `en`, `fr`, and `es`; reusable signal-word labels such as `warning`, `danger`, and `tips` belong in `signal_words` with `copy_key` as the first column and language columns after it; symbols page titles and alt text stay in `page_copy`
...
- a phase2 snapshot is considered valid only when `snapshot_manifest.json` shows that every required core table was requested and synced in the same run: `spec_master`, `spec_footnotes`, `spec_notes`, `spec_titles`, `page_copy`, `symbols_page_copy`, `signal_words`, `symbols_blocks`, and `troubleshooting`, plus the derived `row_key_mapping`; partial table refreshes stay usable through explicit `--data-root` but are not auto-selected as the default snapshot
...
- content-source boundary: manual body text, titles, table headers, prompts, image alt text, default placeholder copy, and output titles must come from RST or phase2/Feishu Base sources; Python and YAML config may keep only field names, enum values, paths, rendering logic, error messages, and CLI help
...
- when `spec_master` is synced from the split source tables, `sync-data` reads `spec_footnotes` as needed and converts Feishu linked-record style footnote refs such as `{"id":"rec..."}` into stable `Footnote_id` values before writing `Spec_Master.csv`
...
- `python build.py process-review-start-queue --config config.us.yaml --data-root .tmp/review-start/phase2` consumes the `sync.phase2.review_init` table, finds rows where `是否进入Review` is checked and `Workflow_action` maps to `Start Review`, resolves the review target from `Document_Key` alone, uses `Build_family` / `Lang` only as optional config-routing hints, groups only the rows whose target config enables `build.queue_by_document_key`, syncs a fresh phase2 snapshot, always re-seeds `docs/_review` from the latest `origin/main` template/data state, force-updates the review branch if it already exists, opens or reuses a PR, then writes back the same `Git_ref`, `PR_url`, `Review_status=InReview`, and cleared `是否进入Review` state to every row in that routed group
- a row is eligible to start review only when `Document_Key` is a non-empty `<MODEL>_<REGION>` value, `是否进入Review` is checked, and `Workflow_action` maps to `Start Review`
...
- `python build.py process-build-queue --config config.us.yaml` consumes the `sync.phase2.document_link` task table, writes `开始构建时间` as soon as a pending row starts, resolves the build config from `Build_family` first and `Lang` second, groups only the rows whose resolved config enables `build.queue_by_document_key`, builds Draft rows as `check -> word -> md`, upgrades Publish rows to `check -> diff-report -> word -> pdf -> md`, uploads the Draft DOCX or Publish PDF to the primary Feishu/wiki destination, can then sync that same primary artifact to DingTalk as an optional mirror, writes the local DOCX release path back to `Document directory`, keeps `Document link` as the DOCX link for Draft and PDF link for Publish, imports the generated Markdown as a Feishu cloud document when the table exposes `飞书云文档`, optionally writes the DingTalk node URL to `Document link_dd`, adds sync notes such as `cloud_doc=ok` into `构建结果`, writes the refresh result back to `data_sync`, and flips the trigger back to `已构建`
- for `build.queue_by_document_key` configs, `Build Draft Package` rows with a non-empty `Lang` are split by normalized language instead of being merged only by `Document_Key`; `Lang=br` / `pt-br` maps to `pt-BR`, and that selected language is forwarded through `check`, `word`, `md`, bundle materialization, and output-path resolution. `config.pt-br.yaml` is now a single-language entrypoint, so Brazil Portuguese draft rows should use `Build_family = pt-br` with `Lang=br` or `Lang=pt-BR` instead of adding an English companion row.
- `process-build-queue` writes `RUNNING | ... started_at=...` to `构建结果` when a row starts, then overwrites it with the final `SUCCESS` or `FAILED` state
...
- queue routing is now `Build_family`-first: use `us-merged`, `eu-merged`, `us-en`, `eu-en`, `us-es`, `us-fr`, `pt-br`, `jp-ja`, or `cn-zh`; `Lang` remains a compatibility field and no longer decides the target when `Build_family` is filled
...
- queue rows may expose `Task_id` as `Document_ID + "_" + Workflow_action`; for Start Review rows, `Document_Key + "_" + Workflow_action` is enough because review creation does not require a versioned `Document_ID`
- when `Document_Key` is exposed as a linked Base field, its API value may be only a linked record id; Start Review chat resolution therefore treats `Task_id` plus `是否进入Review` and `Workflow_action=Start Review` as the stable selector
...
- merged review-init and build-queue rows should use `Build_family = us-merged` / `eu-merged` and may leave `Lang` blank; single-language rows should use the matching single-language family such as `us-en` / `eu-en` / `us-fr` / `us-es` / `pt-br`
- config policy for `build.queue_by_document_key`: enable it for merged whole-book families that intentionally represent one shared manual across languages, such as today's `us-merged`, `eu-merged`, and future `cn-merged`; keep it disabled for single-language families such as `us-en`, `eu-en`, `us-fr`, `us-es`, `pt-br`, `jp-ja`, `cn-zh`, or future `eu-de` / `eu-fr`, which should continue to route one row per `record_id`
- when the queue row includes `Version`, Build Draft Package DOCX/Markdown names use `manual_<model>_<region>_<lang>_<Version>.docx|md`, while Publish queue release artifacts use `manual_<model>_<region>_<lang>_publish_<Version>.docx|pdf|md`
...
- `python build.py md` and `--formats md` reuse the Word bundle HTML path and export Markdown; the exporter prefers a native Pandoc `myst` writer when available, otherwise it emits MyST-compatible CommonMark with pipe tables
...
- the OpenClaw `start-review`, `build-draft`, and `publish` bridges dispatch with the resolved Feishu `record_id`; this keeps GitHub run metadata and final Feishu writeback tied to the exact row selected by `Task_id`, `Document_ID`, or chat context
- the Feishu IM OpenClaw adapter can now resolve config-scoped batch Build Draft Package asks such as `输出JE-1000F的所有欧规说明书文案`, `构建JE-1000F的所有欧规说明书文案`, `基于配置构建JE-1000F的欧规`, or the implicit-all form `构建JE-1000F的欧规说明书文案`; the parser turns that into a `Task_id` prefix such as `JE-1000F_EU_`, keeps only rows whose `Task_id` maps to `Build Draft Package` and whose `是否触发文档构建` is enabled, then dispatches each matching row by `record_id`. When no market is named, asks such as `构建JE-1000F说明书文案` use the broader prefix `JE-1000F_`, so every triggered Build Draft Package row for that model is eligible across markets. Versioned market-level asks such as `构建 JE-1000F_EU_1.0 的欧规说明书文案` keep the same prefix and add `Version=1.0`, so all configured language rows for that version remain eligible. `最新` does not collapse these batch Draft requests by shared `Document_Key`; the trigger checkbox remains the eligibility gate for each language row. The draft workflow concurrency group is also scoped by `queue_record_id`, so batch dispatches do not cancel one another while waiting for GitHub runners. `是否强制刷新数据` remains a build-time row input: if checked, the queue worker refreshes phase2 before building that row.
- `queue-execute` and the targeted Build Draft Package worker both reject exact Document_link rows whose `是否触发文档构建` is not enabled, so OpenClaw no longer reports an empty targeted run as a successful draft build
- `queue-execute` now treats `Start Review` rows that already have `Review_status=InReview` and `Git_ref` as completed and returns those fields without dispatching another Action, so repeated review requests stay idempotent once the selected row has already entered review
- in this repo, the OpenClaw-backed operator identity is **BlockClaw**: the document-build operator for `auto-manual` that works with content blocks, helps run review/build/publish flows, inspects queue rows and failures, and produces manual-ready wording that supports that workflow
...
- desktop deployments that receive Feishu events through the installed OpenClaw gateway can run [`integrations/openclaw/scripts/patch_openclaw_feishu_received_reaction.mjs`](integrations/openclaw/scripts/patch_openclaw_feishu_received_reaction.mjs) before `openclaw gateway` starts, so the native Feishu `Get` reaction is added directly on `im.message.receive_v1` before agent reasoning or queue execution
...
- `python build.py queue-query --config config.us.yaml --query-text "查 JE-1000F_US_0.3 的 Build Draft Package" --json` is the preferred natural-language entry for Phase 2; it applies document_id-first parsing so tokens like `JE-1000F_US_0.3` are treated as exact `Document_ID` before any broader field inference happens. JSON responses include `matched_count`, `returned_count`, `limit`, and `truncated`, so a default-limited broad query no longer looks like a complete inventory.
- for a stable full-count lookup, use `python build.py queue-query --config config.us.yaml --queue-scope document-link --result-contains success --limit 200 --json` plus the same command with `config.ja.yaml`, then count only rows whose `normalized_workflow_action` is `draft` or `publish`; natural-language inventory asks such as `当前所有已构建文档链接` are also resolved as successful `Document_link` rows with a larger default limit instead of a new build action
...
- the same parser also understands spaced operator asks such as `帮我生成 JE-1000F US 0.3 草稿`, document-key-only review asks such as `review JE-1000F_EU`, and failure-reason asks such as `为什么 JE-1000F US 0.3 构建失败`; when it can build an exact `Document_ID`, that exact id still wins over broader inferred filters
...
- broad latest-link asks such as `构建好的文档链接发我` filter to successful `Document_link` rows and return the latest version per `Document_Key`, while inventory asks such as `当前所有已构建文档链接` keep all matching successful rows and rely on the `truncated` metadata to warn when a higher `--limit` is needed
- OpenClaw conversation memory is only a selector hint: status follow-ups re-read Feishu by `record_id`, and if a remembered row was deleted or moved, the adapter reports it as not found and clears that context instead of replaying the old row
...
- the RTD job installs the system `pandoc` package, runs `python build.py md` for each published target, then runs [`tools/readthedocs_source.py`](tools/readthedocs_source.py) to assemble [`docs/_build/rtd/`](docs/_build) as the Sphinx source with a top-level link-only `index.md` manual directory and mirror each manual's image assets under Sphinx `_static/manual-assets/`
...
`Spec_Master.csv` is now a read model rebuilt from Feishu `规格参数明细` (`Page=specifications`) and `页面占位参数` (non-spec placeholder pages). Its first column is `spec_row_key`; `document_key` remains the target dimension. In the two source tables, `Row_key` is derived from `参数名.Row_key` through `Row_key_link`, so editors choose the dictionary row instead of typing the key; `Model` and `Region` are also omitted from the source tables and derived from `document_key` during rebuild. `Page` may now be a comma-separated page list. Use `Product overview` for Product overview-only placeholder rows, and use `Product overview, specifications,` when the same row is shared by both pages. For page-value rows, keep `Row_key` as the concept and use `Slot_key` to describe the placeholder slot. The shared source-text columns are `Row_label_source`, `Param_source`, and `Value_source`; they hold the row's source-manual text. `Source_lang` stores that source-language code explicitly, for example `en`, `ja`, or `zh`, and code no longer infers it from `Region`. `document_key` is the target helper column in `[Model]_[Region]` form; `Source_lang` stays separate. `Row_order` is now a formal column and controls the row display order inside each `document_key + Page + Section`, while `Line_order` is required and controls the order of multiple lines inside one logical row. Visible section defaults can live in `spec_titles.csv section_order`, but if `Spec_Master.csv Section_order` is filled, that explicit value has the highest priority. `Row_label_en`, `Param_en`, and `Value_en` are no longer supported; source-language text must live in `*_source`. The old `project_code` / `项目代码` column has been removed; row targeting now uses `Region` + `Model`. Spec-cell footnotes are now referenced through `Row_label_footnote_refs`, `Param_footnote_refs`, and `Value_footnote_refs`; do not handwrite `①②③` into visible spec text.
  ...
  `symbols_blocks.csv` stores symbols-page table copy, uses `Region` and `Model` to match target manuals like `Spec_Master.csv`, keeps `Source_lang` aligned with the same source-language code pattern, and includes an `image_path` field for the referenced icon asset. Use `block_type=table_row` for normal symbol/meaning rows, and `block_type=signal_row` for the warning/caution/note/tips signal-word table; signal rows use uppercase `symbol_key=WARNING`, `CAUTION`, `NOTE`, or `TIPS`, and their meaning text must be maintained in the row's `text_*` columns. Symbols table headers and signal labels belong in the live `symbols_page_copy` phase2 table, with `copy_key` as the first column and language columns after it. Reusable signal-word labels belong in the live `signal_words` phase2 table, also as `copy_key` plus language columns; use `tips`, not `tip`. Page-level symbols title and image alt text stay in `page_copy`. When the authoring Base provides a `Figure` attachment, `sync-data` downloads it into `data/phase2/_attachments/symbols/` and uses that local file as `image_path`. Leave `Region` / `Model` blank when one symbols row should be shared.
