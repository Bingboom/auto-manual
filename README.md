...
- checking the old engineering Base against the new business Base with the read-only schema parity workflow; its documented aliases (`文档构建表` → `02_文档构建`, `数据入库表` → `01_数据入库`) prevent intentional tenant renames from becoming duplicate-table alerts
- treating the retired `文档构建表.Document link` as replaced by `基线文档` and `飞书云文档` in the business Base, so the parity alert does not request a redundant field
- syncing `main` one-way into [`Bingboom/Hello-Docs`](https://github.com/Bingboom/Hello-Docs) through [`sync-hello-docs.yml`](.github/workflows/sync-hello-docs.yml); the workflow imports the exact source Git tree so file attributes cannot rewrite mirrored blobs; `Hello-Docs` keeps its own GitHub Secrets / Variables for the alternate Feishu and OpenClaw bindings, including the `FEISHU_PHASE2_MODEL_CAPABILITIES_TABLE_ID` table binding, with `FEISHU_BUILD_QUEUE_PAUSED=true` scoped to the mirror Feishu runtime workflows until those bindings are ready; trusted Feishu-triggered review PR checks are auto-approved while ordinary external PRs retain GitHub's approval gate
...
The staged fold index is checked with
...
- structured data changes: Feishu phase2 source tables are the authoring surface; [`data/phase2/`](data/phase2) is a gitignored local snapshot root used by build, review, and publish flows
  `Manual_Copy_Source.csv` is the human-maintained single-language source for reusable page chrome and labels such as LCD / Symbols page titles, table headers, Product overview part labels, Symbols signal labels / meanings, and spec page / section titles. `Localized_Copy.csv` and `spec_titles.csv` are generated from that source plus Translation Memory rows tagged `manual_copy`; do not maintain either file directly online. The US Spanish, French, and Brazilian Portuguese Product Overview templates consume the existing `product_overview.*` keys from `Localized_Copy.csv`; keep the EU raw-LaTeX Product Overview pages on their existing path. Keep long instructional prose in RST templates, model/spec values in `Spec_Master.csv`, normal Symbols row content in `symbols_blocks.csv`, and LCD description variables in `Variable_Defaults.csv` plus `Variable_Lang_Overrides.csv`. Image alt text is derived from existing page titles, panel titles, `symbol_key`, or generated signal labels instead of duplicated as copy rows. LCD status-word bolding is driven by `Status_Words.csv`, a local snapshot of Translation Memory rows marked `是否为 status word=Y`, so status words are not duplicated in `Localized_Copy.csv` or the LCD icon table.
...
`Spec_Master.csv` is now a read model rebuilt from Feishu `规格参数明细` (`Page=specifications`) and `页面占位参数` (non-spec placeholder pages). Its first column is `spec_row_key`; `document_key` remains the target dimension. In the two source tables, `Row_key` is derived from `参数名.Row_key` through `Row_key_link`, so editors choose the dictionary row instead of typing the key; `Model` and `Region` are also omitted from the source tables and derived from `document_key` during rebuild. `Page` may now be a comma-separated page list. Use `Product overview` for Product overview-only placeholder rows, and use `Product overview, specifications,` when the same row is shared by both pages. For page-value rows, keep `Row_key` as the concept and use `Slot_key` to describe the placeholder slot. The shared source-text columns are `Row_label_source`, `Param_source`, and `Value_source`; they hold the row's source-manual text. `Source_lang` stores that source-language code explicitly, for example `en`, `ja`, or `zh`, and code no longer infers it from `Region`. `document_key` is the target helper column in `[Model]_[Region]` form; `Source_lang` stays separate. `Row_order` is now a formal column and controls the row display order inside each `document_key + Page + Section`, while `Line_order` is required and controls the order of multiple lines inside one logical row. Visible section defaults can live in `spec_titles.csv section_order`, but if `Spec_Master.csv Section_order` is filled, that explicit value has the highest priority. `Row_label_en`, `Param_en`, and `Value_en` are no longer supported; source-language text must live in `*_source`. The old `project_code` / `项目代码` column has been removed; row targeting now uses `Region` + `Model`. Spec-cell footnotes are now referenced through `Row_label_footnote_refs`, `Param_footnote_refs`, and `Value_footnote_refs`; do not handwrite `①②③` into visible spec text.
  The eight shared-language charging-method templates read their safety-critical solar-panel range from the `页面占位参数` row `Page=charging_methods`, `Row_key=pv_input_range`, `Slot_key=value`, exposed as `|PV_INPUT_RANGE|`. Keep each language value's approved spacing and dash style; do not restore a literal range in RST. Production rows are added only through the F6 approval path.
  Those shared charging templates also read the model-specific connector name from `dc_input_connector/value` as `|DC_INPUT_CONNECTOR|`; the matching shared UPS templates read `ups_transfer_time/value` as `|UPS_TRANSFER_TIME|`. The zero-millisecond incompatibility cautions remain authored prose, not transfer-time data. Seed production rows only through F6 and preserve localized units such as Ukrainian `мс`.
  ...
  The two dormant known `Spec_Master` value repairs are tracked in [`data/spec_master_value_repairs.csv`](data/spec_master_value_repairs.csv); the repair pass reads that CSV instead of carrying target/value patches in Python.
  ...
  Keep merged-language and single-language prefaces as separate components: the merged US stack uses [`00_preface.rst`](docs/templates/page_shared/en/00_preface.rst), while the AU English-only manifest uses [`00_preface_single_language.rst`](docs/templates/page_shared/en/00_preface_single_language.rst). A single-language manifest must not consume a preface containing foreign language-tag blocks.
...
- Editable IDML CJK text uses explicit character runs driven by `CJK_FONT_FAMILY_TOKEN` (`idml_font_family_cjk`) instead of inheriting the Latin Gilroy paragraph family. The token reuses the existing Arial Unicode MS resource and is not a layout-geometry row, so Latin-only package bytes and approved reference-layout hashes remain unchanged.
- IDML line and coarse text-width budgets use one deterministic East Asian Width contract: `W`/`F` characters count as full-em glyphs, combining marks add no width, and ambiguous-width characters remain narrow. This replaces duplicated Latin-only `0.52 × point size` estimates without consulting workstation fonts; native InDesign finalize/parity remains the final fit authority.
