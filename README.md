...
- `lcd_icons`, `troubleshooting`, `symbols_blocks`, `variable_defaults`, and `variable_lang_overrides` are first-class phase2 sync tables; LCD icon `figure` attachments sync into `data/phase2/_attachments/lcd_icons/`, symbols `Figure` attachments sync into `data/phase2/_attachments/symbols/`, symbols signal-word rows can use `block_type=signal_row`, and LCD variable placeholders resolve through `Variable_Defaults.csv` plus exact language rows in `Variable_Lang_Overrides.csv`
- `troubleshooting` renders from `data/phase2/troubleshooting_blocks.csv`; keep rows in the TROUBLESHOOTING Base table with `Region`, `Model`, `Is_latest`, `error_code`, and `corrective_measures_*` columns, while page title, intro, headers, table widths, and header-row settings stay owned by the renderer
...
- `symbols_blocks.order` must be filled with unique values such as `1, 2, 3...`; `table_row` entries are sorted and split evenly into two columns, while optional `signal_row` entries must provide the four uppercase `symbol_key` values `WARNING`, `CAUTION`, `NOTE`, and `TIPS`
...
- structured data changes: Feishu phase2 source tables are the authoring surface; [`data/phase2/`](data/phase2) is the preferred local snapshot root, with [`data/phase1/`](data/phase1) kept as the legacy baseline
