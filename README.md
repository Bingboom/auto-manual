...
- `build.py check --source review` still validates target identity and generated-page recipe inputs, but it does not block on unrelated latest `Spec_Master` rows that the review bundle will not consume; runtime-source validation remains strict for target rows
...
- when the Start Review workflow is dispatched with one explicit `record_id` but the GitHub worker cannot re-read that row as pending from the bound Feishu view, the worker records that as a structured failure instead of a silent no-op success
