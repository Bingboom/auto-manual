...
- against the control-layer plan, the repo is now effectively at message-control `Phase 4 / publish confirmation in progress`: `queue-query`, `queue-execute`, Build Draft Package dispatch, Start Review dispatch, and structured status replies are in place; Publish now requires explicit confirmation instead of a bare dispatch
