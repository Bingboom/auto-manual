...
- for a stable full-count lookup, prefer explicit filters over broad natural-language text: use `python build.py queue-query --config config.us.yaml --queue-scope document-link --result-contains success --limit 200 --json` plus the same command with `config.ja.yaml`, then count only rows whose `normalized_workflow_action` is `draft` or `publish`; this avoids missing rows when broad phrases like `document link` or `构建好的文档` do not recall every market consistently
...
- broad built-link asks such as `构建好的文档链接发我` now filter to successful `Document_link` rows and return the latest version per `Document_Key`, so old `1.0` links do not mask newer `1.1` builds
- for operator counting, do not rely on broad built-link phrasing alone; the fixed full-count method is `queue-scope=document-link + result-contains=success + high limit`, then separate `draft` and `publish` by `normalized_workflow_action`
