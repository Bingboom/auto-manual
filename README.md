...
- the three GitHub-hosted Feishu workers now share [`.github/actions/feishu-common-setup/action.yml`](.github/actions/feishu-common-setup/action.yml) plus [`scripts/validate_required_env.sh`](scripts/validate_required_env.sh), so Python/Node/pandoc/lark setup and required-env validation only need to change in one place; the shared setup now installs Pandoc from the official release action instead of `apt-get`, which avoids the multi-minute mirror stalls seen on GitHub-hosted runners
