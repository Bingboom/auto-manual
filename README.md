...
- `operator_union_id`, `DingTalk_session_key`, and `钉钉会话键` all select the same per-row DingTalk session key; if a row sets `alice`, the worker expects `<session_root>/alice.json`
- when a row is supposed to sync DingTalk but the selected session source is missing, `process-build-queue` now fails that row before build starts and writes the missing-session reason back to `构建结果`
...
- `python build.py queue-query --config config.us.yaml --queue-scope all --document-id JE-1000F_US_0.3 --json` is the Phase 2 natural-language helper surface for resolving Feishu queue rows into one concrete `record_id`, `Workflow_action`, `Git_ref`, `Document link`, `构建结果`, and, for Publish rows when available, `HTML_link`
