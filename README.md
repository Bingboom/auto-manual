...
- for the planned DingTalk provider, [`tools/dingtalk/spike_cli.py`](tools/dingtalk/spike_cli.py) is the generic manual Phase 0 smoke helper; it uses the official App-Only token flow by default and lets you inject product-specific list/update/upload endpoints before any repo-integrated provider refactor starts
...
- [`tools/dingtalk/openapi_upload_cli.py`](tools/dingtalk/openapi_upload_cli.py) is the first official App-Only upload smoke helper; it expects one bound `operator_union_id` plus a target node URL, then runs `uploadInfos/query -> signed upload -> commit` without browser-session headers
- `python build.py dingtalk-control-config --config config.us.yaml --operator-union-id union_xxx --target-node-url https://alidocs.dingtalk.com/i/nodes/... --json` is the repo-owned Feishu control-plane command for the long-lived `dingtalk_openapi` worker path; OpenClaw should call this command to validate/store the bound operator plus default target row
...
- `python build.py process-build-queue --config config.us.yaml` consumes the `sync.phase2.document_link` task table, writes `开始构建时间` as soon as a pending row starts, resolves the build config from `Build_family` first and `Lang` second, groups only the rows whose resolved config enables `build.queue_by_document_key`, builds the generated Word file, then publishes that artifact through the configured sink before writing the local Word path back to `Document directory`, the resolved link back to `Document link`, optionally dual-writing the DingTalk node URL to `Document link_dd` when that field exists and the sink is DingTalk, a timestamped status string to `构建结果`, the refresh result back to `data_sync`, and the trigger back to `已构建`
...
- the long-lived `dingtalk_openapi` worker path now has a repo-owned Feishu control-plane surface: GitHub Secrets hold `DINGTALK_CLIENT_ID`, `DINGTALK_CLIENT_SECRET`, and `DINGTALK_CORP_ID`, while OpenClaw should bind and persist `operator_union_id` plus the default DingTalk target node through `build.py dingtalk-control-config`; `process-build-queue` then reads that Feishu control row on each run before uploading
- add `FEISHU_PHASE2_DINGTALK_CONTROL_TABLE_ID`, `FEISHU_PHASE2_DINGTALK_CONTROL_VIEW_ID`, and optionally `FEISHU_PHASE2_DINGTALK_CONTROL_RECORD_ID` when you want the worker and control command bound to one dedicated Feishu config row
- for the GitHub-hosted `dingtalk_openapi` worker path, also set repository secret `AUTO_MANUAL_ARTIFACT_SINK_PROVIDER=dingtalk_openapi`; the build and draft queue workflows now export `AUTO_MANUAL_ARTIFACT_SINK_PROVIDER`, `DINGTALK_CLIENT_ID`, `DINGTALK_CLIENT_SECRET`, `DINGTALK_CORP_ID`, and the `FEISHU_PHASE2_DINGTALK_CONTROL_*` bindings into the runner environment
...
- when a checked DingTalk row also contains `DingTalk_target_node_url`, that row-level target wins first; if the row field is blank, the `dingtalk_openapi` worker falls back to the Feishu control-row `default_target_node_url`, while the legacy `dingtalk_alidocs_session` path still falls back to `DINGTALK_DOCS_TARGET_NODE_URL`
...
- use [`scripts/run_feishu_im_webhook_adapter.sh`](scripts/run_feishu_im_webhook_adapter.sh) or [`scripts/run_feishu_im_webhook_adapter.ps1`](scripts/run_feishu_im_webhook_adapter.ps1) to start that adapter with the repo root, default config, and state-file path prewired; then point the Feishu app's event-subscription request URL at `/feishu/events`, keep callbacks in plain mode, subscribe to `im.message.receive_v1`, and publish the app change
- for the smallest macOS deployment shape, use [`integrations/openclaw/feishu-im-webhook-adapter/deploy/README.md`](integrations/openclaw/feishu-im-webhook-adapter/deploy/README.md) plus the bundled `cloudflared` and `launchd` templates to keep one local adapter process and one stable HTTPS tunnel online
- against the control-layer plan, the repo now has the full repo-local Phase 2 control stack: `queue-query`, `queue-resolve-action`, `queue-execute`, structured failure replies, explicit Publish confirmation, and a standalone Feishu IM webhook adapter. The remaining gaps are deployment hardening, shared state, and encrypted callback support.
