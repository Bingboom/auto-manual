...
- the OpenClaw `build-draft` bridge now reuses one recent shared draft-queue worker for rapid multi-language dispatch bursts, so `en + es + fr` batch asks do not race three GitHub queue runs against the same global draft queue slot
...
- [`.github/workflows/feishu-build-queue.yml`](.github/workflows/feishu-build-queue.yml) builds queue-driven Publish rows, stages the latest publish HTML under [`site/publish-latest/dist/`](site/publish-latest/dist), then runs `vercel pull`, `vercel build`, and `vercel deploy --prebuilt`; if the `Document_link` table includes `HTML_link`, the same workflow writes that deployed Vercel URL back to the originating publish row, while `RTD_link` is filled from the repository variable `AUTO_MANUAL_RTD_URL` when that field exists; the raw deploy URL is preserved in `publish_meta.json` plus `openclaw-run-metadata` because GitHub summaries/logs may mask it
...
- if you want publish queue rows to expose that stable public RTD entry, set the repository variable `AUTO_MANUAL_RTD_URL` and add an `RTD_link` field to `Document_link`
