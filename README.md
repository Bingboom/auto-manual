...
- the Feishu IM OpenClaw adapter can now resolve batch Build Draft Package asks such as `输出JE-1000F的所有欧规说明书文案`; the parser turns that into a `Task_id` prefix such as `JE-1000F_EU_`, keeps only rows whose `Task_id` maps to `Build Draft Package` and whose `是否触发文档构建` is enabled, then dispatches each matching row by `record_id`. `是否强制刷新数据` remains a build-time row input: if checked, the queue worker refreshes phase2 before building that row.
