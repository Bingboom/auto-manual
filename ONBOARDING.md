...
| InDesign 终饰（IDML→成品 PDF） | **仅操作者 Mac**（真 InDesign + `tools/idml/indesign_finalize.jsx`），不在任何 CI | **有版本锁**（`tools/idml/indesign_version_pin.json`，finalize 启动时比对、不匹配拒跑）+ 第二主机 runbook（[`code-as-doc/dev/indesign_second_host_runbook.md`](code-as-doc/dev/indesign_second_host_runbook.md)）；第二主机 `ArriettyMac-mini.local` **已于 2026-07-21 通过首次端到端验证**（1.6 handoff zip finalize,与主力机基线逐项一致,见 runbook §3 登记）——此腿恢复路径已实证可用 |
