...
import re
import sys
from tools.utils import spec_master_auditing as _auditing
from tools.utils import spec_master_lookup as _lookup
from tools.utils import spec_master_mapping as _mapping
from tools.utils import spec_master_repairs as _repairs
from tools.utils.spec_master_shared import (
    DERIVED_MULTILINE_PLACEHOLDERS as _DERIVED_MULTILINE_PLACEHOLDERS,
    KNOWN_ROW_LABEL_REPAIRS as _KNOWN_ROW_LABEL_REPAIRS,
    KNOWN_VALUE_REPAIRS as _KNOWN_VALUE_REPAIRS,
    LEGACY_PAGE_VALUE_BINDINGS as _LEGACY_PAGE_VALUE_BINDINGS,
    LEGACY_PAGE_VALUE_KEYS_BY_SIGNATURE as _LEGACY_PAGE_VALUE_KEYS_BY_SIGNATURE,
    PageValueBinding,
    ProductNameMatch,
    SLOT_KEY_VALUE_ALIASES as _SLOT_KEY_VALUE_ALIASES,
    SOURCE_COLUMN_NAMES as _SOURCE_COLUMN_NAMES,
    SOURCE_LANGUAGE_NORMALIZATION as _SOURCE_LANGUAGE_NORMALIZATION,
    SOURCE_SHARED_BASES as _SOURCE_SHARED_BASES,
    SECTION_NORMALIZATION_RULES as _SECTION_NORMALIZATION_RULES,
    SECTION_ORDER_BY_SECTION as _SECTION_ORDER_BY_SECTION,
...
def resolve_spec_value_from_rows(
    ...
    *,
    ...
    return _lookup.resolve_spec_value_from_rows(
...
def collect_matching_spec_rows(
    ...
    *,
    ...
    return _lookup.collect_matching_spec_rows(
        sys.modules[__name__],
        rows,
        model=model,
        region=region,
...
def collect_spec_value_matches_from_rows(
    ...
    *,
    ...
    return _lookup.collect_spec_value_matches_from_rows(
        sys.modules[__name__],
        rows,
        model=model,
        region=region,
...
def resolve_template_substitutions_from_rows(
    ...
    *,
    ...
    return _lookup.resolve_template_substitutions_from_rows(
        sys.modules[__name__],
        rows,
        model=model,
        region=region,
...
def resolve_product_name_from_rows(
    ...
    *,
    ...
    return _lookup.resolve_product_name_from_rows(
        sys.modules[__name__],
        rows,
        model=model,
        region=region,
...
def resolve_product_name_from_spec_master(
    ...
    *,
    ...
    return _lookup.resolve_product_name_from_spec_master(
        sys.modules[__name__],
        spec_master_csv,
        model=model,
        region=region,
...
def resolve_template_substitutions_from_spec_master(
    ...
    *,
    ...
    return _lookup.resolve_template_substitutions_from_spec_master(
        sys.modules[__name__],
        spec_master_csv,
        model=model,
        region=region,
...
def read_spec_master_rows(spec_master_csv: Path) -> list[dict[str, str]]:
    return _lookup.read_spec_master_rows(sys.modules[__name__], spec_master_csv)


def audit_spec_master_rows(rows: list[dict[str, str]]) -> SpecMasterAuditResult:
    return _auditing.audit_spec_master_rows(sys.modules[__name__], rows)


def audit_spec_master_csv(spec_master_csv: Path) -> SpecMasterAuditResult:
    return _auditing.audit_spec_master_csv(sys.modules[__name__], spec_master_csv)


def _row_issue_map(issues: tuple[SpecMasterAuditIssue, ...]) -> dict[int, list[SpecMasterAuditIssue]]:
    return _auditing._row_issue_map(sys.modules[__name__], issues)


def normalize_spec_master_rows(rows: list[dict[str, str]]) -> SpecMasterNormalizationResult:
    return _auditing.normalize_spec_master_rows(sys.modules[__name__], rows)


def normalize_spec_master_csv(spec_master_csv: Path) -> SpecMasterNormalizationResult:
    return _auditing.normalize_spec_master_csv(sys.modules[__name__], spec_master_csv)


def build_template_row_key_mapping_rows(
    rows: list[dict[str, str]],
) -> tuple[dict[str, str], ...]:
    return _mapping.build_template_row_key_mapping_rows(sys.modules[__name__], rows)
...
def build_row_label_row_key_mapping_rows(
    ...
    return _mapping.build_row_label_row_key_mapping_rows(
        sys.modules[__name__],
        rows,
...
def build_row_label_row_key_mapping_markdown(
    ...
    return _mapping.build_row_label_row_key_mapping_markdown(sys.modules[__name__], mapping_rows)


def build_template_row_key_mapping_markdown(
    mapping_rows: tuple[dict[str, str], ...],
) -> str:
    return _mapping.build_template_row_key_mapping_markdown(sys.modules[__name__], mapping_rows)


def repair_known_spec_master_values(rows: list[dict[str, str]]) -> SpecMasterRepairResult:
    return _repairs.repair_known_spec_master_values(sys.modules[__name__], rows)


def repair_known_spec_master_csv(spec_master_csv: Path) -> SpecMasterRepairResult:
    return _repairs.repair_known_spec_master_csv(sys.modules[__name__], spec_master_csv)
