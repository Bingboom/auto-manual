...
def _write_myst_sphinx_scaffold(
    markdown_path: Path,
    *,
    title: str,
    ...
    if profile == WEB_PRESENTATION_PROFILE:
        copy_web_stylesheet(source_dir)
        conf_lines.extend(
            [
    ...
    *,
    ...
    if presentation_profile == WEB_PRESENTATION_PROFILE:
        ...
        if protected_figures:
            ...
            pandoc_source = Path(temporary_input.name) / bundle_html.name
            pandoc_source.write_text(protected_html, encoding="utf-8")
    try:
        subprocess.run(
            [
                pandoc_bin,
