...
from ..page_copy import require_page_copy, symbols_copy_map_from_blocks
...
def _copy_for_symbols(
    ...
    copy = symbols_copy_map_from_blocks(blocks or [], lang)
    if copy:
        missing = [key for key in _SYMBOL_COPY_KEYS if key not in copy or not copy[key].strip()]
        if missing:
            raise ValueError(
                f"symbols copy_row missing required key(s) for lang={lang!r}: "
                + ", ".join(missing)
            )
    else:
