...
| Troubleshooting 可编辑故障表 | 两列表格；窄错误码列；0.25pt 内线、0.57pt 外框；法/西错误码表头为两行 | `HBTroubleshootingTable` | `comp_trouble_left_ratio` / `comp_trouble_steps_pad_tb` / `comp_data_table_{header,row}_height` / `type_{data_table,trouble_code}_*` / `comp_table_outer_arc` | `components/prose_table.TroubleshootingTableStyle` 统一读取共享 token；EN/FR/ES 原生行高基线加本地化换行增量，提前确定固定可编辑组高；外壳继续 `AutoSizingType=Off` | △ 英文保留 240pt floor；法/西完整 12 行和 `FE` 由组件测试守护，最终可见性只以原生 InDesign 0 overset 为准；母版行 minima、光学校正、线宽和 import allowance 仍是 renderer calibration，改动必须重跑 native preflight |
