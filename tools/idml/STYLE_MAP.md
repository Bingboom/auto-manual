...
| spec/LCD/正文数据表外框 | 深色圆角描边 + 灰色表头/label 列 + 内部网格；Auto Resume 左整列（含左表头）灰底、右整列白底；单元格文字纵向居中；文字离左边界一个字符位 | `HBSharedDataTable`；spec、LCD、Auto Resume、Troubleshooting 只声明列与行语义，列为 `m` / 内容盒为 `[c]` | `comp_table_outer_arc` / `comp_table_outer_rule` / `comp_data_table_*` / `comp_table_text_indent` | `components/rounded_table` 公共容器：“圆角背景 + 方形可编辑内容框 + 四角弧外遮罩 + 顶层描边”；正文正式表占满正文栏宽，字符缩进只落在 cell；LCD 行先按内容最低高度和共享预算分配，`AutoGrow` 负责真实字体重排，`indesign_finalize.jsx` 再按原生行高收紧每段 LCD 壳体 | ✅ 灰色单元格填充不会穿出圆角；表格仍保持原生可编辑；LCD 按同一分段规则生成多个完整圆角表，LaTeX/InDesign 共用 `lcd_table_layout.py`；英/法/西短行先压缩，仍超出预算时整行换到下一页，不以压缩到溢流为代价；源编号保持稳定，批准参考合同可单独声明显示顺序、显示编号、跨行编号和排版密度角色，映射不完整时失败关闭 |
...
| Key Combinations 可编辑操作图 | 三列表格骨架 + 4 组按钮/时钟链接图；表头、按钮名、`+`、时长、操作和功能文案均在最上层独立文字框 | `HBKeyCombinationTable`；源保持三列表格，只声明按钮组合、操作和功能语义 | 共享 `comp_key_table_*` + `idml_key_*`；`idml_key_clock_gap` 同时控制第二列边线→时钟和时钟→时长的一个字符位；FR/ES 只用 `lang_<lang>_idml_key_panel_*` 覆盖受治理的高度与位置 | `components/key_combinations.KeyCombinationStyle` 从同一参数表解析样式；`prose_table.body_data_table_kind` 按四组稳定按钮角色路由；底层网格/链接图与顶层 27 个可移动文字框分层输出 | ✅ 英/法/西复用同一组件和列骨架，不复制逐语言绘制代码；approved-reference 缺任一链接资产时构建硬失败，只有未批准的通用渲染上下文可原子退回原生可编辑表；审批页面计划失效时同样硬失败，不能把该组件推入 overset 后继续交付 |
| App Add Device 可编辑组合图 | 手机截图 + 配对面板为底层链接图；2.1/2.2 与三个按钮标签为最上层独立文字框 | Product Overview 首表固定槽位提供三语 `main_power` / `dc_usb` / `ac` 基础标签；批准合同保存 App 显示变体 | `idml_app_*` 九个共享文字/自适应 token；无逐语言绘制分支 | `control_labels` 校验语言+角色同源绑定，`prose_flow` 兼容基础/冻结显示标签去重，`components/reference_figure.AppFigureStyle` 统一量测；两个 App 组合均带全栏不可见边界，防止 InDesign 按最左素材重新归一化 x 坐标 | ✅ 普通正文和西语 2.3 不会被误吞；approved-reference 缺角色、变体、素材或 token 硬失败；英法西标签均可编辑、可移动且不改变 IR 内容哈希 |
| US 可编辑封底 | 公司/地址、电话、邮箱/网址、分隔线与 QR 均为原生对象；QR 外框 37.2pt、AI 码面 30pt 居中留 quiet zone | `HBSpecialBackCover` | 批准 reference-layout 合同的 `editable_components.back_cover` 几何 | `page_placed.add_back_cover_page` 将公司与地址拆成独立文字框；二维码链接只允许已批准 AI candidate，外层圆角框由 InDesign 原生绘制 | ✅ 文字仍可编辑；生产包不链接参考 PDF、参考 QR 或历史整页封底；AI QR payload 由资产证据与测试守护 |
| Meaning of Symbols 表 | 圆角描边 + 首列 K05 浅灰 + 内部网格；左右表按各自内容高度收口 | `HBSymbolTable` / `HBSymbolTwoColumnTablesSplit` | `comp_symbol_*` | `pages._symbols_signal_table` / `_symbols_icon_table`；`indesign_finalize.jsx` 累加原生 `row.height`，同步收紧内容框和圆角外框；法/西信号词沿用紧凑标签密度，超长词按可用宽度单行适配；warning box 标签单独按语言和词宽自适应，英文保持固定字号；符号续页的法/西长文案按行高自适应字号，避免 WEEE 文案 overset | ✅ 英/法/西首列统一浅灰；`fitted_symbol_table_shells` 记录最终化数量；短列不再保留底部白带；法语 `AVERTISSEMENT`、`ATTENTION`、`REMARQUE`、`CONSEILS` 不再在黑底标签内换行 |
| inbox 三卡 | 圆角卡片 + 13.785pt 圆形编号，10.912pt Medium 白字 | inbox card tcolorbox | `comp_inbox_card_arc` + `idml_inbox_overflow_*` / `lang_{lang}_*` | `components/inbox` + page03 卡片框；符号续页的标题、卡片、图像和提示条走同一语言布局 profile；编号框和字均纵向居中 | ✅ 编号字形中心与圆心残差 0.003pt；西语符号续页不再复用英文固定坐标 |
...
| LCD 标签/说明正文 | `idml_lcd_{first,continuation}_*` + `lang_{lang}_...` + 批准合同 `typography_role` | 标签以 7.0pt Bold 为基准；正文按首段/续段、语言和密度角色读取共享 token | 源内容不带排版例外；批准参考合同只选择语义密度角色，字体值仍由样式 token 统一管理 |
