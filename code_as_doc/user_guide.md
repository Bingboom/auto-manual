# JE-1000F JP User Guide

Updated: 2026-03-11

This guide describes the real working flow for `manual_je1000f_jp`.
It assumes the final editable manual lives under `docs/_review/JE-1000F/JP/` after review starts.

Target in this guide:

- product: `JE-1000F`
- region: `JP`
- config: `config.je1000f.jp.yaml`
- final Word output: `docs/_build/JE-1000F/JP/word/manual_je1000f_jp.docx`
...
2. Review working layer
   - `docs/_review/JE-1000F/JP/index.rst`
   - `docs/_review/JE-1000F/JP/page/*.rst`
   - `docs/_review/JE-1000F/JP/generated/JE-1000F/*.rst`
   - `docs/_review/JE-1000F/JP/overrides/**`
   - purpose:
...
3. Runtime publish layer
   ...
   - `docs/_build/JE-1000F/JP/html/**`
   - `docs/_build/JE-1000F/JP/word/**`
   - `docs/_build/JE-1000F/JP/pdf/**`
   - purpose:
...
- after review starts, edit `docs/_review/JE-1000F/JP/**`
- do not use `docs/_build/**` as the editing surface
...
Edit `docs/templates/page_jp/*.rst` or CSV when the change should be reusable for other products too.
...
- a real product parameter update in `Spec_Master.csv`
...
- `docs/_review/JE-1000F/JP/overrides/_static/**`
...
```powershell
python build.py rst --config config.je1000f.jp.yaml --model JE-1000F --region JP --source runtime
```

What this does:

- reads `docs/templates/page_jp/*.rst`
- reads `Spec_Master.csv` and other CSV data
...
```powershell
python build.py review --config config.je1000f.jp.yaml --model JE-1000F --region JP
```
...
- if `docs/_review/JE-1000F/JP/` does not exist, it is created
...
```powershell
python build.py review --config config.je1000f.jp.yaml --model JE-1000F --region JP --refresh-review
```
...
- do not keep editing `docs/templates/page_jp/*.rst` for daily JE-1000F JP review work
- do not keep editing `docs/_build/JE-1000F/JP/rst/**`
- do keep editing `docs/_review/JE-1000F/JP/**`
...
```powershell
python build.py check --config config.je1000f.jp.yaml --model JE-1000F --region JP
```
...
If `docs/_review/JE-1000F/JP/` exists, the review content is overlaid onto the runtime bundle before export.

So these commands now build from review by default:

```powershell
python build.py rst --config config.je1000f.jp.yaml --model JE-1000F --region JP
python build.py html --config config.je1000f.jp.yaml --model JE-1000F --region JP
python build.py word --config config.je1000f.jp.yaml --model JE-1000F --region JP
python build.py pdf --config config.je1000f.jp.yaml --model JE-1000F --region JP
```

If you want to be explicit, you can force review mode:

```powershell
python build.py word --config config.je1000f.jp.yaml --model JE-1000F --region JP --source review
```

If you want to temporarily ignore review and preview only template/data output:

```powershell
python build.py word --config config.je1000f.jp.yaml --model JE-1000F --region JP --source runtime
```
...
## 10. Stage G: Baseline and Normal Review Rounds
...
## 11. Stage H: Export the Revision Record
...
```powershell
python build.py diff-report --config config.je1000f.jp.yaml --tracked-root docs/_review/JE-1000F/JP --from-ref HEAD~1 --to-ref HEAD
```

If this is the first baseline and you do not want one-time Added rows in the report:

```powershell
python build.py diff-report --config config.je1000f.jp.yaml --tracked-root docs/_review/JE-1000F/JP --from-ref HEAD~1 --to-ref HEAD --ignore-initial-adds
```
...
## 12. Stage I: Publish the Final Release

Use `publish` as the formal release command.

```powershell
python build.py publish --config config.je1000f.jp.yaml --model JE-1000F --region JP
```

What `publish` does:

1. runs `check` against review content
2. exports the diff report from `docs/_review/JE-1000F/JP`
...
## 13. Stage J: Build the Final Word Document Directly
...
- `auto` prefers `docs/_review/JE-1000F/JP/` when it exists
...
## 14. Full Example for `manual_je1000f_jp`
...
### 14.3 Intentional reseed from template/data
...
- Editing `docs/_build/JE-1000F/JP/rst/**` and expecting the edits to survive
...
- Continuing to edit `docs/templates/page_jp/*.rst` for target-only review comments
...
seed from template/data once -> `review` once -> edit `_review` -> commit -> `publish` -> `push`
