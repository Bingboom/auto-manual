...
It documents the current build layout, maintenance rules, the review bundle layer under `docs/_review/<model>/<region>/`, and the current review-first publishing flow.
...
3. Review working layer
   ...
   - `docs/_review/<model>/<region>/overrides/**`
...
- After review starts, use `docs/_review/...` as the daily editing surface for that target.
- Edit templates only when the change should be shared by multiple manuals.
- Edit CSV when product parameters change.
- Treat `docs/_build/...` as generated runtime output.
...
1. `python build.py rst|html|word|pdf|all|review|check|publish`
...
6. `tools/gen_index_bundle.py` materializes the runtime bundle
7. the runtime bundle is written to `docs/_build/<model>/<region>/rst/`
8. if source mode is `auto` or `review` and a review bundle exists, review content is overlaid onto the runtime bundle
9. `docs/index.rst` is refreshed to point at all existing bundle roots
10. `html`, `word`, and `pdf` outputs are built from the prepared bundle when requested
11. `python build.py review` seeds `docs/_review/<model>/<region>/` from the runtime bundle when review starts
12. `python build.py check` runs config/layout validation, prepares the bundle, and scans for minimal content issues
...
- `docs/_review/**` is emitted as a review-first snapshot
...
2. Edit `docs/_review/<model>/<region>/**`
...
python build.py review --config config.je1000f.us.yaml --model JE-1000F --region US
python build.py review --config config.je1000f.us.yaml --model JE-1000F --region US --refresh-review
python build.py check --config config.je1000f.us.yaml --model JE-1000F --region US
python build.py publish --config config.je1000f.us.yaml --model JE-1000F --region US
...
- exports revision reports to `reports/version_tracking/<model>/<region>/` by default
...
Because `docs/_review/**` is now the preferred review surface, you can keep cleaner RST history per target.
...
5. Rebuild preview outputs from that review bundle and commit again.
6. Run `publish` for the formal release output, or run `diff-report` separately when needed.
...
- `review` prepares `docs/_build/<model>/<region>/rst/**` from template/data
- then it seeds `docs/_review/<model>/<region>/**`
...
- one model across all tracked regions:
  `docs/_review/JE-1000F`
- one model and one region:
  `docs/_review/JE-1000F/US`
- temporary runtime-only comparison:
  `docs/_build/JE-1000F`
...
```powershell
python build.py diff-report --config config.je1000f.us.yaml
python build.py diff-report --config config.je1000f.us.yaml --tracked-root docs/_review/JE-1000F --from-ref HEAD~1 --to-ref HEAD
python build.py diff-report --config config.je1000f.us.yaml --tracked-root docs/_review/JE-1000F --from-ref HEAD~1 --to-ref HEAD --ignore-initial-adds
```

Example report export for one region:

```powershell
python build.py diff-report --config config.je1000f.us.yaml --tracked-root docs/_review/JE-1000F/US --from-ref HEAD~3 --to-ref HEAD
```
...
```powershell
python build.py diff-report --config config.je1000f.us.yaml --tracked-root docs/_review/JE-1000F/US --from-ref <old_commit> --to-ref <new_commit>
```
...
- `reports/version_tracking/JE-1000F/*_files.csv`
- `reports/version_tracking/JE-1000F/*_files.html`
- `reports/version_tracking/JE-1000F/*_pages.csv`
- `reports/version_tracking/JE-1000F/*_pages.html`
- `reports/version_tracking/JE-1000F/*_fields.csv`
- `reports/version_tracking/JE-1000F/*_fields.html`
- `reports/version_tracking/JE-1000F/*_index.html`
- legacy compatibility aliases remain available as `reports/version_tracking/JE-1000F/*.csv` and `*.html`
...
- `fields`: structured field/value changes extracted from list-tables and `Label: Value` lines
  For generated `spec_*.rst` pages, the report now also tries to fill `source_row_key`, `source_section_key`, `source_line_order`, and `source_csv_line` from `Spec_Master.csv`.
...
- `source_row_key`: the matched source row in `Spec_Master.csv`
- `source_section_key`: the matched source section in `Spec_Master.csv`
...
1. open `reports/version_tracking/JE-1000F/*_index.html`
...
- `docs/templates/contracts/05_operation_guide_eu.yaml`
- `docs/templates/contracts/12_app_setup.yaml`
- `docs/templates/contracts/12_app_setup_eu.yaml`

Current scope:

- contracts are matched by source template path from `config.pages`
- `check` validates required placeholders directly against `Spec_Master.csv`
...
- fallback values in `conf_base.py` no longer hide missing product-specific spec data
...
- edit `docs/_review/<model>/<region>/**`
...
3. the review bundle appears under `docs/_review/<model>/<region>/`
...
`docs/_review/**` becomes the target editing source after review starts.
`docs/_build/**/**/rst/**` remains the runtime publish bundle behind the final outputs.
