...
class PublicationCatalogTests(unittest.TestCase):
    ...
    def test_sphinx_grouped_home_and_single_manual_dropdown(self):
        ...
        hub = json.loads(home.split('window.hubData = ', 1)[1].split('</script>', 1)[0].rstrip(';'))
        self.assertEqual(len(hub['products']), 1)
        self.assertEqual({item['code'] for item in hub['products'][0]['languages']}, {'en', 'fr'})
