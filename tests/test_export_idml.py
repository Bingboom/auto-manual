...
class FlowModeTests(unittest.TestCase):
    """Single-story book flow (Word-like continuous threading)."""

    BUNDLE = ROOT / "tests" / "fixtures" / "idml_bundle"

    def _build_flow(self) -> Path:
        import subprocess
        out = Path(tempfile.mkdtemp()) / "flow.idml"
        r = subprocess.run(
            [sys.executable, str(ROOT / "tools" / "export_idml.py"),
             "--model", "JE-1000F", "--region", "US",
             "--data-root", str(FIXTURE_DATA_ROOT),
             "--bundle-root", str(self.BUNDLE), "--flow", "--out", str(out)],
            capture_output=True, text=True)
        self.assertEqual(r.returncode, 0, r.stderr + r.stdout)
        return out

    def test_flow_is_single_story_threaded_across_all_frames(self) -> None:
        out = self._build_flow()
        with zipfile.ZipFile(out) as zf:
            stories = [n for n in zf.namelist() if n.startswith("Stories/")]
            self.assertEqual(stories, ["Stories/Story_st_flow.xml"])
            parents, frames = set(), 0
            import re as _re
            for n in zf.namelist():
                if not n.startswith("Spreads/"):
                    continue
                for m in _re.finditer(r'ParentStory="([^"]+)"', zf.read(n).decode()):
                    parents.add(m.group(1))
                    frames += 1
            self.assertEqual(parents, {"st_flow"})
            self.assertGreater(frames, 1)

    def test_flow_package_is_valid(self) -> None:
        from tools.export_idml import check_idml
        self.assertEqual(check_idml(self._build_flow()), [])
