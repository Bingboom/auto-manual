...
const DOCUMENT_ID_PARTS_RE = /^([A-Za-z]{1,8}-\d{3,5}[A-Za-z]?)_([A-Za-z]{2,3})(?:_([A-Za-z]{2}))?(?:_(\d+(?:\.\d+)*))?$/;
...
function safeSelectorFromContext(conversationContext) {
  ...
  if (match) {
    ...
    return [model, region.toUpperCase(), version || ""].filter(Boolean).join(" ");
  }
  if (/^[A-Za-z]{1,8}-\d{3,5}[A-Za-z]?_[A-Za-z]{2,3}$/.test(documentKey)) {
    const [model, region] = documentKey.split("_");
    return [model, region.toUpperCase()].join(" ");
  }
