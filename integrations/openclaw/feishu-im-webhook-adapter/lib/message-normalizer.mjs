...
const SEND_LINK_FOLLOWUP_RE = /^(发|发下|发一下|发送|send)$/i;
...
function isSendLinkFollowup(messageText) {
  return SEND_LINK_FOLLOWUP_RE.test(compactText(messageText));
}
...
export function normalizeIncomingMessage({ messageText, localProfile = null, conversationContext = null } = {}) {
  ...
  const sendLinkFollowup = isSendLinkFollowup(aliasExpandedText);
  ...
  const shouldUseBatchContext = Boolean(
    (statusIntent || sendLinkFollowup) &&
      !executionIntent &&
      !hasTarget &&
      (hasPreviousReference || sendLinkFollowup) &&
      batchRows.length > 1
  );
