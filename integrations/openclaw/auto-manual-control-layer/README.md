...
is needed. On the Feishu IM adapter the same shape is deterministic: a
single-record build replies "已受理（处理中）" immediately, dispatches with
`--no-wait`, then a background poll sends a follow-up — `已完成` once the Base row
is fresh, or `任务正在处理中` if it is still building when the poll window ends.
Either way the operator can re-ask "这个好了没" to re-query the result on demand.
