...
- Use [`.agents/skills/pdf-web-sideload/SKILL.md`](.agents/skills/pdf-web-sideload/SKILL.md) to land a Web book that must ship before its structured intake (spec extraction, templating) is done: PyMuPDF-extract the shipped PDF (never `pdftotext` — it interleaves double-column text), arrange a MyST `md/` bundle named to the config's output template **declaring every component with its `tools/manual_md_directives.py` directive** (`{callout}`, `{spec-table}`, `{troubleshooting}`, …; hand-written component HTML and inline styles are refused, because only the directive layer emits the structure the shared stylesheet keys off), stage it with `build.py web-sideload` (two-stage gate: author-plane lint + directive build, then a compile step and a second strict build under RTD's own `myst_parser,tools.rtd_portal` extension set — what gets staged is the compiled output; plus the mandatory `source_kind: "pdf_sideload"` marker, no pipeline evidence, an unconditional `整本未结构化` debt entry), then collect it exactly like a pipeline book with `build.py web-assemble`. Payoff is mechanical: once `spec-sheet-structured-intake` lands real data, an ordinary `web-release` run for the same target overwrites the sideloaded `publish_meta.json` in place. Not for a book that already has a review branch or phase2 data (use `web-release`), and not for a brand-new model/region/lang target (use `new-region-line` first).
