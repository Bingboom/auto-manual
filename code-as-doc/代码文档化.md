...
- `code_optimization_log.md`
  - 每次结构优化/重构的变更记录、影响范围、验证结果。
- `spec_master_user_guide.md`
  - `Spec_Master.csv` 在构建链路中的作用、字段定义、Word 构建必需字段。
- `tests/README.md`（如存在测试策略变化）
  - 测试组织方式、执行命令、覆盖边界。
...
必须更新：
- `code_optimization_log.md`：记录目标、改动文件、回归结果。
- `code_style_guide.md`：若规范/分层约束发生变化，必须同步。
...
- `code_style_guide.md`：若引入新的层级边界或开发约束，必须同步。

条件更新：
- 若涉及 `Spec_Master.csv` 读取逻辑或字段语义：同步更新 `spec_master_user_guide.md`。
- 若测试策略变化：同步更新 `tests/README.md`。

## 3.3 配置变更（`config.yaml` 字段新增/删除/重命名）

必须更新：
- `README.md`：配置字段说明、默认值、优先级、迁移方式。
- `code_optimization_log.md`：记录兼容性影响和回归结论。

## 3.4 数据契约变更（CSV 字段、过滤条件、必填规则）

必须更新：
- `spec_master_user_guide.md`：字段定义、必填项、筛选逻辑、示例。
- `README.md`：如果构建链路的输入源、路径或行为变化，必须同步。
- `code_optimization_log.md`：记录契约变更影响范围与验证结果。

## 3.5 构建链路变更（html/pdf/word 任一流程）

必须更新：
- `README.md`：构建顺序、工具依赖、命令示例。
- `code_optimization_log.md`：记录为何改、改了什么、如何验证。
...
- 规则 2：任何“架构/分层/共享逻辑”变化，必须更新 `code_style_guide.md` 或 `code_optimization_log.md`。
- 规则 3：任何“CSV 字段/过滤条件”变化，必须更新对应数据指南（`spec_master_user_guide.md`）。
...
- [ ] 若改动构建链路，已记录在 `code_optimization_log.md`。
- [ ] 若改动规范/架构边界，已更新 `code_style_guide.md`。
- [ ] 若改动 `Spec_Master.csv` 契约，已更新 `spec_master_user_guide.md`。
...
- 中改动：同步更新 `README.md + code_optimization_log.md`。
- 大改动（重构）：同步更新 `README.md + code_style_guide.md + code_optimization_log.md`，必要时补专题指南。
