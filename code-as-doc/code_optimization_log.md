...
- introduced [`data/phase2/Localized_Copy.csv`](../data/phase2/Localized_Copy.csv) as the phase2 short-copy snapshot for page titles, table headers, alt text, and Product overview labels; LCD status words stay in `lcd_icons_blocks.csv` rows marked `是否为 status word=Y` for renderer prefix bolding
