...
- introduced [`data/phase2/Localized_Copy.csv`](../data/phase2/Localized_Copy.csv) as the phase2 short-copy snapshot for page titles, table headers, non-row alt text, and Product overview labels; LCD status words are marked in Translation Memory with `是否为 status word=Y` and exported to `Status_Words.csv` for renderer prefix bolding, while Symbols image alt text is derived from `symbol_key` or signal-row labels
