...
Use `Manual_Copy_Source.csv` for reusable source-language short text, including LCD / Symbols page titles, table headers, Symbols signal labels / meanings, Product overview labels, and spec titles. `Localized_Copy.csv` and `spec_titles.csv` are generated from those source rows plus Translation Memory rows tagged `manual_copy`; RST templates continue to reference generated copy with `{{ copy:<copy_key> }}` and missing copy remains a build/check error. Image alt text is derived from existing page titles, panel titles, `symbol_key`, or generated signal labels instead of maintained as separate copy rows. Normal Symbols row content stays in `symbols_blocks.csv`; signal rows there own structure and targeting, not visible copy. LCD status words stay in Translation Memory rows marked `是否为 status word=Y` and are exported to `Status_Words.csv` for prefix bolding.
