...
本文件是本轮 34 项任务及范围变更新增项（REV-35 起）的唯一执行状态入口。方案保留范围，路线图保留方向和优先级，
...
| <a id="rev-35"></a>REV-35 | 语料管理 | — | 语料快照每月导出并保留历史（只存计数，不含语料原文），系统建设页显示与上期对比；连续两期在册即可验收；缺期记为无数据，不补造 | planned | 待指派 / — |
...
| <a id="rev-45"></a>REV-45 | 多 Agent 设计储备 | REV-44；操作者重新批准重启 | 当前只保留盘点和设计参考，不实施。未来重启时重新确认实际交接瓶颈、任务边界、验收与维护成本，再选择一项真实任务验证顺序交接；不作为网页化、语料、SSOT、IR 或 IDML 验收的前置 | deferred | 操作者 2026-09-26「还没有准备要做多 agent」「这个先不合入 #1279 需要先降级」；[#1279](https://github.com/Bingboom/auto-manual/pull/1279) 转草稿，第二版 [设计储备](agent_task_state_design_2026-09.md) 与[第一版历史方案](multi_agent_dispatch_design.md) 均不作为实施指令；重启责任=夏冰，条件=已有流程仍无法解决持续交接/重复工作瓶颈，且重新批准具体范围；REV-20 按原台账独立推进 |
...
REV-35–43 由 2026-09-25 范围变更新增（见第 3 节范围变更表），初始均为 planned：
REV-35–38 语料建设与管理，REV-39–40 IR 共享，REV-41–43 骨架拓展。
REV-44–45 由同日第二次范围变更新增，初始均为 planned：REV-44 SSOT 来源登记，REV-45 多 Agent 调度与任务续接（依赖 REV-44）。
...
|---|---|---|---|---|
| 2026-09-26 / 夏冰 | REV-45；#1279 | 操作者明确尚未准备建设多 Agent，并要求「这个先不合入 #1279 需要先降级」 | REV-45 deferred；仅保留设计储备，#1279 转草稿、不合入；网页化、语料、SSOT、IR 和 IDML 任务按各自原有依赖继续，本项不成为前置 | 夏冰；现有台账/PR/交接说明无法解决持续交接瓶颈时，再明确范围、试点与维护成本并批准重启 |
| 2026-09-25 / 夏冰 | 新增 REV-35–43 | 操作者定当前重点为说明书网页化与语料库建设与管理，后续为 IR 共享与骨架拓展，并同意把三条线补进台账（原话「1,2」，答复系统建设页调整提议）；此前三条线没有台账条目，系统建设页无法显示其进度。来源：翻译记忆库只读画像（语料）、[PR #1083](https://github.com/Bingboom/auto-manual/pull/1083) 迁移计划（IR，未合入）、骨架库设计与 [PR #1101](https://github.com/Bingboom/auto-manual/pull/1101)（骨架，未合入） | 允许：REV-35–38 与 G1–G3 并行；REV-39–43 登记为下一步，不进入 G1–G4 串行门，也不阻塞 Web 门；IDML 原生验收仍按 REV-20/26 | 夏冰；各行依赖满足后开工，开工时按第 1 节规则指定执行人、目标和验收方式 |
| 2026-09-25 / 夏冰 | 新增 REV-44–45；REV-39 开始 | 外部评审第三项「SSOT 来源登记」先行，多 Agent 调度与任务续接记进台账并依赖它（原话「是不是。要先做。SSOT」「先记进台账（推荐）」「SSOT 草稿和 REV-39 同时开始（推荐）」），并同步到系统建设页（「这些要。推到系统建设页面去的」）；系统建设页“Agent 执行”卡原写“多 Agent 调度与任务续接尚未建设”，此前没有台账条目 | REV-39 只读摸底与 REV-44 设计草稿现在开始；REV-40 仍待 REV-20；REV-45 待 REV-44 定稿 | Claude / REV-45 在 REV-44 验收后开工 |

## 4. 当前续接点

- REV-45 暂缓（2026-09-26）：操作者尚未准备建设多 Agent，要求 #1279 先不合入并降级。保留设计材料，草稿不触发实施；当前继续使用已有台账、PR 和续接说明。不增加任务记录/认领/审批凭据设施，不阻塞已有生产任务。后续重启条件见 REV-45 行；下方历史“可开始设计”等记录不构成实施授权。
...
- 范围变更（2026-09-25）：新增 REV-35–43，均为 planned。语料建设与管理 REV-35–38 为当前重点，可与 G1–G3 并行；IR 共享 REV-39–40、骨架拓展 REV-41–43 为下一步。系统建设页按网页化、语料库、IR 共享、骨架拓展四条线显示进度。
