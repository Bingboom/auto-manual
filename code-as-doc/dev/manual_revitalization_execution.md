...
|---|---|---|---|---|---|
| <a id="rev-01"></a>REV-01 | WP1 | — | 两站配置、部署提交、版本、旧新 URL 同时点清单；不以截图代替查询 | planned | 待指派 / — |
| <a id="rev-02"></a>REV-02 | WP1 | REV-01 | HTML_link、印刷二维码、交付链接原值及最新/历史语义对应表 | planned | 待指派 / — |
| <a id="rev-03"></a>REV-03 | WP1 | REV-02, REV-06, REV-08, REV-10 | 获批迁移范围、兼容验证、旧站停止独立更新证据、映射恢复记录；保留 publish 分支 | planned | 待指派 / — |
| <a id="rev-04"></a>REV-04 | WP1 | REV-01 | 带时间/来源/维度的型号×区域×语言覆盖清单；解释 24/47/52 差异 | planned | 待指派 / — |
| <a id="rev-05"></a>REV-05 | WP1 | — | 命令/调度/归档/同步能力矩阵，逐项标已有、缺口、待核实；不恢复撤回命令 | planned | 待指派 / — |
| <a id="rev-06"></a>REV-06 | WP1 | REV-02, REV-04, REV-05 | M0 差异表；每个差异有解释、责任人和处理决定，不补造队列历史 | planned | 待指派 / — |
| <a id="rev-07"></a>REV-07 | WP2 | REV-06, REV-08 | M1/M2 缺口切片；幂等、登记失败独立重试、人工字段保护及授权回读证据 | planned | 待指派 / — |
| <a id="rev-08"></a>REV-08 | WP2 | REV-05 | 批准、合入、部署、线上版本核验的字段与真实回执映射；复用既有能力 | planned | 待指派 / — |
| <a id="rev-09"></a>REV-09 | WP2 | REV-07, REV-08 | 代表目标修订/发布/恢复证据；已有演练复用，新演练区分隔离环境和生产 | planned | 待指派 / — |
| <a id="rev-10"></a>REV-10 | WP2/WP3 | REV-02, REV-04, REV-05 | M4/M5 入口映射历史、统计窗口/过滤版本及目录快照；不覆盖旧统计 | planned | 待指派 / — |
| <a id="rev-11"></a>REV-11 | WP3 | — | 真实候选清单及语言责任人审核；批准、拒绝、待定分开，授权入库须回读 | planned | 待指派 / — |
| <a id="rev-12"></a>REV-12 | WP1 | REV-04, REV-06 | 大陆访问实测范围、IT 待办及具体下一批目标；域名阻塞有负责人和恢复条件 | planned | 待指派 / — |
...
| 门 | 范围与通过条件 | 当前记录 |
|---|---|---|
| G1 发布基线 | REV-01–12 验收；两条发布路径、旧链接语义、对账及恢复证据符合方案 0–30 天出口 | 未验收 |
...
- 当前实施任务：无。方案及台账由 PR #1188 提交评审，不能算任一 REV 任务完成。
- 下一候选：**REV-01**，只读核对两站项目配置、构建提交、文档身份及版本，形成同一时点清单。
- 开工前：核对本 PR 实际状态、领取执行责任并确定证据位置；后续只读核查可在已授权范围内进行。
- 尚未授权的动作：本轮没有站点迁移、线上表写入、定时任务或生产发布的具体操作授权。
- 可独立准备：REV-05 现有能力盘点、REV-11 候选清单调查；不能据此跳过原有源表写入/译文审核门。
- 本轮实际交付：任务登记和执行规则，无实施验收。

每轮结束覆盖更新上面的当前续接点；重要历史结论由 Git 和所链接的验收记录保留。
证据至少记录：任务 ID、范围、源提交/版本、PR 状态、验证命令及结果、实际输出位置、
验收人/日期、剩余事项。被阻塞时同时写下解锁责任人、下一动作及复查日期/事件。
