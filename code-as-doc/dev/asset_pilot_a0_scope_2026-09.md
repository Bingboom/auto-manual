...
- 本文性质：只读盘点记录（同日后续的母版实测与建槽位另见 §5、§8 的指引）。
...
计划 §5 A0 原有一项"确认设计联系人及实际使用的编辑器"。
**操作者 2026-09-08 明确取消这一项**：不绑联系人，改为在多维表给每个插图资产建槽位，
谁画好图都能直接往槽位里传。已执行，见[槽位建成记录](asset_slots_bitable_2026-09.md)。

母版形态仍记录在案：`.ai`，`asset_sources.csv` 标注 `pdf-illustrator-compatible`。
...
- `JS-100I / EU / en`：13 张 Web 整图，A0 盘点时 `asset_registry.csv` **0 行**，完全绕过注册表；
  已于同日补入注册表并建槽位，见[槽位建成记录 §3.5](asset_slots_bitable_2026-09.md)；
...
| 确认设计联系人及实际使用的编辑器 | ⊘ 操作者取消该项，改为建槽位，见 §5 |
| 只盘点试点，不重跑全仓资产审计 | ✅ 未跑全量审计；线上查询为只读全表计数 |

A0 的盘点项全部有证据；设计联系人一项由操作者取消而非未完成。

## 8. A0 盘点本身未做的事

本节的边界只描述 A0 这次盘点。同日后续两轮（母版实测、建槽位）另有各自的边界说明，
见 [A1 实测 §6](asset_pilot_a1_master_editability_2026-09.md) 与
[槽位记录 §5](asset_slots_bitable_2026-09.md)。

- A0 盘点阶段为纯只读：未下载母版、未写线上表、未改任何 CSV。
- 未修改模板、manifest、review 派生物或任何发布产物。
- 未联系设计同事、未发送任何文件（该方向随后被操作者取消，见 §5）。
