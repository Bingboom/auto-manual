...
2. **Duplicate / contained `en`** — within each table: exact duplicates; for TM, per-model copies and target agreement; and **containment** (one row's `en` a substring of another). Containment is *not* automatically redundancy — combined cells (LCD `On/Blink/Off`, multi-bullet blocks, paragraphs) are usually the real unit; only near-duplicates (bullet/punct-only diff) and fully-covered artificial joins are safe to merge.
...
- TM: 1 empty-source row (`recvlfSn0Qmb17`, since gone); per-model duplicate sentences **merged 2026-06-01** — 22 truly-redundant groups collapsed (31 rows removed, `Model` multi-select union, `Glossary_term` links preserved). Of the 6 genuinely-divergent groups, **5 resolved 2026-06-01** (4 merged to the chosen value with per-language maintenance/audit logs — incl. replacing an untranslated DE placeholder + fixing its typo; `bypass-mode` **unified to the detailed version across both models** per owner decision, short variant removed), **1 kept distinct** (`CHARGING`: `section_title` CARREGAMENTO vs status Carregando — different contexts, logged). Whitespace **normalized 2026-06-01** (27 TM + 8 GL cells, internal newlines preserved). Numeric/symbol noise rows **removed 2026-06-01** — 24 TM rows whose `en` had no letter of any script (15 bare numbers `9`–`24`, 8 callout circled numbers `①`–`⑧`, 1 stray fragment `60%).`), each translated to itself = zero TM value. Contained-sentence review **2026-06-01** — of 130 sentences whose `en` is a substring of a longer one (terms excluded), **10 near-duplicate rows + 1 artificially-joined warning cell removed** (kept the richer/cleaner row; absorbed-cell noted in constituents' EN log); the rest **kept**, since combined cells are usually the legitimate translation unit (LCD `On/Blink/Off` descriptions, error messages, warranty/FCC paragraphs, numbered procedures) — *被包含 ≠ 冗余*. **TM now 725 rows** (from the 839 audit baseline). Backups under `~/tm_*_backup_2026-06-01.json` / `~/corpus_*_2026-06-01.md`.
