...
Status: PR 0–7 complete; PR 8 in progress
...
| PR 8 — Overview ComponentSpec and target geometry | [ ] | [ ] | `refactor/renderers-overview-component` | — | — | — | three-language composite/live fallback, mobile completeness | Implementation started from verified `origin/main@98498dbc`; target geometry and source-semantic ownership re-inventoried below before edits. |
...
### Deferred baseline debt discovered during acceptance

The real DOCX export is empty on both the `origin/main@407f697c` baseline and
this PR's output. The assembled Word bundle contains the expected EN/FR/ES
editable Inbox tables, but its complete HTML document also contains an empty
`<main></main>` element. Pandoc's full-HTML reader selects that empty element
as the document body and ignores the populated content outside it. This is not
an Inbox regression and must not broaden PR 7; PR 9 owns removal of the empty
body-selection trap plus a real DOCX structural and rendered comparison.
...
PR 9 also owns the pre-existing Word export defect characterized in PR 7:
remove the empty `<main></main>` body-selection trap before Pandoc conversion,
then prove the resulting DOCX contains and renders the populated bundle rather
than accepting bundle-HTML semantics alone.
