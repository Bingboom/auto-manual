...
| MA-016 | `JE-1000F_US_1.8` Publish 解阻所需修复 PR（含 `fix/publish-review-overlay-repro` 及本登记行），仅限 review overlay 可复现性修复与完成该记录消费所必需的后续修复；不越过 F6、workflow、公开 CLI、Base schema 等独立 gate | 「授权你合入 能达到此次目标的所有pr」2026-08-10 | `JE-1000F_US_1.8` Publish 成功交付并完成线上表读回验收，或操作者撤销 | 生效 |
