...
| MA-022 | PR #975（`docs/milestone-m-r0-coverage-rebaseline`，含本登记行）——仅限 R0 覆盖率重算、`JBP-2000B_EU` 身份与母版裁决证据；必须满足 17/17 检查全绿、无 changes-requested、无未解决 review thread 后 squash merge。**不含** #974；不授权提前执行 R1a/R1b 或写入规格、占位、文案、资产等后续线上数据 | 「授权你绿了合入」2026-08-30 | #975 合入即失效 | 生效 |
...
| MA-026 | **严格按 #990 → #989 顺序**:#990(`docs/close-ma-025`,含本登记行,进 main——关闭 MA-025 并记上两个 squash SHA)、#989(`fix/bp-jp-data-story-font-sinks` → **base 是 `feat/bp-jp-target-onboarding`,不是 main**——把 `add_trouble_story` / `add_spec_story` 接回共享 story 汇聚点 `add_story_parts`,让 JP 规格页与故障排除页不再落在 `Arial Unicode MS` 宿主系统字体上;`composed_ja` golden 主动重基线 14 行,逐行确认全为 CJK 字体替换,非 ja 三个变体逐字节不变)。#990 先合,让授权与 MA-025 的关闭同时落在 main 上。**不含**:`feat/bp-jp-target-onboarding` 本身进 main(仍未开 PR、R3c 审批门未完成——资产入库审批与法务/保修确认仍缺、原生 InDesign 视觉轮未跑);**不含 #987**(版式令牌闸门,仍须等 BP JP 整条进 main 之后);不含新 JP 页是否声明故障排除语义那项待裁决改动;不越过 F6 线上源表写入、`.github/workflows/**`、公开 CLI 行为、依赖版本、Base schema 等独立 gate | 「合入 #989 和 #990」2026-09-01 | 两个 PR 全部合入即失效 | 生效 |
