...
| MA-025 | BP JP 产线 CI 解阻批次,**严格按 #988 → #985 顺序**:#988(`fix/gitleaks-record-id-allowlist`,含本登记行,进 main——把飞书 `record_id` 业务键加入 secret-scan 白名单,附负对照)、#985(`fix/bp-jp-layout-token-prefix` → **base 是 `feat/bp-jp-target-onboarding`,不是 main**——七行读不到的 `lang_jp_*` 改成规范前缀 `lang_ja_*`、修掉两处改动 JE-1000F US 字节的共享 TOC 改动、修正分支自带的失败测试)。#988 必须先合:#985 的 secret-scan 红就是它修的那一条,且 `.gitleaks.toml` 要经 main 进入该分支才不会与后续 main 合并冲突。**不含**:`feat/bp-jp-target-onboarding` 本身进 main(仍未开 PR、仍有未裁决发现——JP 字体只接通 4 个 story 汇聚点中的 2 个、新 JP 页未声明故障排除语义);**不含 #987**(版式令牌闸门)——它进 main 后会拒绝该分支现存的 `lang_jp_*` 行,必须等 #985 合入后再合,否则 BP JP 构建直接失败;不越过 F6 线上源表写入、`.github/workflows/**`、公开 CLI 行为、依赖版本、Base schema 等独立 gate | 「你先把 #985 合进 BP JP 分支让它能过 CI(配合 #988 解掉 secret-scan) 然后 继续基于 PR #974 去做」2026-09-01 | 两个 PR 全部合入即失效 | 生效 |
