...
| 2 | 家族 config + 骨架 | `configs/config.<family>.yaml`（照 `config.bp-us.yaml`；`language_family` 写语言范围、`queue_requires_target_match: true`）+ blueprint / region-profile → `tools/skeleton_resolve.py` emit。末页形态从 blueprint 的 `back` superset 里由 `region_profile.terminal_slots` 选择（可为空；顺序仍由 blueprint 决定），不要用型号/区规判断分支。⚠️ 本八步默认输出语言**已注册**；全新语言要先在 `tools/lang_registry.py` 加一行 `LanguageSpec`，那套流程归 [`new-region-line` skill 的 setup-map §2](../../.agents/skills/new-region-line/references/setup-map.md) 所有，本文不复述 | 基本机械；⚠️ `allowed_foreign_identity_literals` 是**整表替换**不是追加 |
