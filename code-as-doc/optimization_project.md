...
| 工作包 | 状态 | 下一步及依赖 |
|---|---|---|
| WP1 发布现状、入口整合与对账基线 | next | 先只读复核 HT-Manuals / HT-Doc、版本及旧链接，输出同一时点基线；迁移操作待具体范围批准 |
| WP2 发布登记与恢复闭环 | deferred | WP1 确认差异后，复用已有生命周期/回执，只补登记幂等、失败重试及对账缺口；在线写入独立确认 |
| WP3 运营与译文首轮维护 | deferred | 数据访问及审核责任明确后可与 WP1 并行；复用 D1–D4 决策，验证统计口径和实际 TM 复用 |

本表是 WP 状态维护入口；交付、验收、失败处理和责任分工由方案第十章定义。
