...
- run [`../scripts/audit_hello_docs_binding.sh --report-only`](../scripts/audit_hello_docs_binding.sh) to check source/mirror tree parity, the source sync token, mirror variables, required mirror secret names, and optional Feishu IM / OpenClaw entries without exposing secret values
