...
- `sync-data`: for any phase2 table that has an `Is_Latest` column, only rows where `Is_Latest=TRUE` are written into the snapshot; this repo-side filter is enforced even if the upstream Feishu view is not strict enough
...
- `listen-sync-data`: start a local Feishu long-connection listener for the phase2 source tables; when `spec_titles / spec_footnotes / spec_notes / symbols_blocks / spec_master` changes, it coalesces those events and runs `sync-data` locally
- that source-table listener now also follows a successful `sync-data` run with `sync-review --sync-scope params` for every review bundle that already exists locally, so the review working layer keeps tracking the newest synced phase2 data
- `listen-phase2-events`: start one shared Feishu long-connection listener for both source-table auto-sync and `Document_link` immediate-build events; use this when the same machine needs both behaviors because the current `lark-cli event +subscribe` allows only one subscriber per app
- the local event-listener entrypoints share one single-instance lock, so launching `listen-phase2-events`, `listen-sync-data`, or `listen-build-queue` again while one is already running will exit cleanly instead of opening a second competing Feishu long connection
- local listeners now fail fast if the app/bot cannot call `/open-apis/drive/v1/files/{base_token}/subscribe`; the current `lark-cli event +subscribe` path only supports bot, so the base must explicitly grant the app/bot enough permission to register and receive docs change events
- the local `Document_link` listener no longer depends only on a single checkbox-diff payload; it performs a startup catch-up scan for already-pending immediate rows and re-checks the current queue on any add/edit event for the queue table
- do not blur the three execution roles:
  - local manual build/sync commands: default to the local `lark-cli auth login` user identity
  - remote GitHub Actions queue worker: always bot identity
  - local long-connection listeners: bot-scoped only for the live event stream transport, because `lark-cli event +subscribe` currently supports bot only
- that distinction matters operationally: adding a remote bot workflow must not be described as changing the identity of ordinary local commands; the only local bot-bound surface is the Feishu event stream itself
...
- `process-build-queue`: consume `sync.phase2.document_link` rows where `是否触发文档构建 = Y`, run one fresh `sync-data` pull for `data/phase2`, write `开始构建时间` immediately when one row is picked up, resolve the matching config family from `Document_Key + Lang`, run `check + word`, upload the generated DOCX to Feishu Drive, try to move that uploaded file into the current wiki knowledge-base container, write the local DOCX path into `Document directory`, write the final link into `Document link`, write a timestamped build status into `构建结果`, and flip the trigger back to `已构建` on success
- `Doc_phase` on `Document_link` controls which build path the queue uses: `Draft` runs `check --source review` plus `word --source review`, `Publish` runs `build.py publish`, and blank keeps the legacy `check + word` behavior
- once a target already has a review bundle, the review-based build entrypoints now auto-run `sync-review --sync-scope params` before `check/word/html/pdf/all/publish`, so Draft and Publish builds refresh review-side parameter content before exporting documents
- if your only goal is "queue-triggered build with the newest phase2 data", prefer `process-build-queue` directly and skip the local listener setup; the queue now refreshes phase2 data for itself before it builds
- `process-build-queue` also supports queue scoping: use `--doc-phase draft|publish` to consume one stage only, and add `--record-id <record_id>` when a workflow should build exactly one `Document_link` row
...
- [`../scripts/listen_sync_data.ps1`](../scripts/listen_sync_data.ps1): Windows listener wrapper for `listen-sync-data`; it restores the local Node/npm path plus the `FEISHU_PHASE2_*` user env vars and writes run logs into [`../.tmp/sync-data-listener/`](../.tmp/sync-data-listener)
- [`../scripts/listen_phase2_events.ps1`](../scripts/listen_phase2_events.ps1): Windows listener wrapper for the shared phase2 listener; it restores the local Node/npm path plus the `FEISHU_PHASE2_*` user env vars and writes run logs into [`../.tmp/phase2-event-listener/`](../.tmp/phase2-event-listener)
- `listen-build-queue`: start the push-based Feishu long-connection listener, register the current `Document_link` base for docs events through the local `lark-cli` user identity, keep the long connection on the app bot identity, and trigger `process-build-queue` immediately when the `是否立即构建` checkbox is checked on a `Document_link` row
...
- [`../.github/workflows/feishu-build-queue.yml`](../.github/workflows/feishu-build-queue.yml): GitHub-hosted Publish queue worker for the remote repo; it runs on a 5-minute schedule plus `workflow_dispatch`, bootstraps `lark-cli` with `FEISHU_APP_ID/FEISHU_APP_SECRET`, sets `FEISHU_PHASE2_IDENTITY=bot`, and consumes only `Doc_phase=Publish` rows from `main`
- [`../.github/workflows/feishu-draft-build-queue.yml`](../.github/workflows/feishu-draft-build-queue.yml): GitHub-hosted Draft queue worker; dispatch it against the PR branch ref and it consumes only `Doc_phase=Draft` rows so the generated review document matches PR branch content
- when you intentionally split stages, keep the ownership strict:
  - `Doc_phase=Draft` belongs to the PR branch and should be dispatched to `feishu-draft-build-queue.yml` with the PR head branch as the GitHub `ref`
  - `Doc_phase=Publish` belongs to `main` and should be consumed only by `feishu-build-queue.yml`
...
- [`../.github/workflows/phase2-word-preview.yml`](../.github/workflows/phase2-word-preview.yml): pull-request validation workflow for runtime Word previews; it syncs `data/phase2` once from Feishu, then runs `check + word` for the explicit target matrix on the PR branch and uploads the generated `.docx` files as GitHub Actions artifacts
...
- if the move step fails, the queue now preserves the latest uploaded Drive URL in `Document link`, preserves the local DOCX path in `Document directory`, clears `是否立即构建` so local startup catch-up does not loop forever, and keeps the row in a failed state until the wiki permission issue is fixed
...
- for local source-table auto-sync on Windows, prefer starting [`../scripts/listen_sync_data.ps1`](../scripts/listen_sync_data.ps1) at login instead of trying to call your local machine from a Feishu cloud workflow
- if the same machine also needs `Document_link` immediate-build listening, start [`../scripts/listen_phase2_events.ps1`](../scripts/listen_phase2_events.ps1) instead of running the sync and build listeners separately
