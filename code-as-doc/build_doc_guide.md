...
- **InDesign same-source web edition:** after the `md` build, `.readthedocs.yaml` runs [`../tools/render_web_edition.py`](../tools/render_web_edition.py), which projects the manual IR — the *same* `manual-ir/v1` source that the IDML export consumes (`tools/idml/ir_projection.py`) — into a print-layout page-card web document at `docs/_build/<model>/<region>/webedition/` (`body.html` + `assets/` + `manifest.json`). `readthedocs_source.py` then grafts it in as the **primary** catalog entry ("InDesign edition"), with the flow HTML manual kept as the secondary "HTML edition" link. The web edition text reflows with a web fallback font stack (the print Gilroy face is not redistributable), so line breaks intentionally diverge from the InDesign artifact — the shared truth is the content and page sequence, not per-line typography; the pixel-authoritative layout remains the InDesign/PDF delivery. The renderer uses [`../tools/idml/web_edition.py`](../tools/idml/web_edition.py) and PyMuPDF (already pinned in `requirements.lock`) to rasterize placed-PDF art (cover/overview). It is best-effort: if the IR cannot be built it logs and skips, and the catalog keeps the HTML-only entry so the RTD build stays green
