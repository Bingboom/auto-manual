...
- set the mirror repo variable `FEISHU_BUILD_QUEUE_PAUSED=true` until the alternate Feishu credentials and table/view bindings are present; `feishu-build-queue.yml` skips the scheduled queue job only in `Bingboom/Hello-Docs` while this mirror variable is true, so a same-named variable in `Bingboom/auto-manual` does not change the source repo queue behavior
