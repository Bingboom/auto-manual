...
- signal rows must include the four rendered `symbol_key` values `warning`, `caution`, `note`, and `tips`; add `danger` as a signal row for alert-label recognition when needed. Maintain one `label_*` value per language for visible signal labels and Word/HTML rewrite detection. The `aliases_*` columns are compatibility mirrors of `label_*`, not separate maintained copy; put old variants or editorial context in `notes`. These rows own the visible signal token, so do not duplicate signal-word content in `Localized_Copy.csv` or Python constants
- normal symbol image alt text is derived from `symbol_key`, and signal image alt text is derived from the corresponding signal-row `label_*`; do not maintain `symbols.*.alt` rows in `Localized_Copy.csv`
