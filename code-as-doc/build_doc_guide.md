...
- the remote GitHub Draft/Publish workers now follow the same sink split: when repository secrets provide the DingTalk browser-session values, the workflow auto-switches to `dingtalk_alidocs_session`; otherwise it stays on the default Feishu/wiki sink
...
- if you switch the queue artifact sink to DingTalk, set `AUTO_MANUAL_ARTIFACT_SINK_PROVIDER=dingtalk_alidocs_session` plus `DINGTALK_DOCS_A_TOKEN`, `DINGTALK_DOCS_XSRF_TOKEN`, and `DINGTALK_DOCS_COOKIE` with optional `DINGTALK_DOCS_TARGET_NODE_URL` and `DINGTALK_DOCS_BX_V`; `DINGTALK_DOCS_TARGET_NODE_URL` is only the default target, and checked rows with `DingTalk_target_node_url` can override it or supply the target on their own
