...
- `process-review-start-queue`: consume `sync.phase2.review_init` rows where `是否进入Review` is checked and `Review_status` is empty / `NotStarted`, using the same Feishu table/view binding as `Document_link` by default when Review Init intentionally reuses that queue table, sync the latest phase2 snapshot, create or reuse the review branch, seed `docs/_review`, push the branch, create or reuse the PR, then write back `Git_ref`, `PR_url`, set `Review_status=InReview`, and clear `是否进入Review`
...
- [`../.github/workflows/feishu-start-review.yml`](../.github/workflows/feishu-start-review.yml): GitHub-hosted review-init worker for the remote repo; it consumes the review-init rows, creates or reuses the review branch, seeds `docs/_review`, pushes the branch, and writes back `Git_ref` plus `PR_url`. When Review Init and `Document_link` share one Feishu table/view, this worker reads the same `FEISHU_PHASE2_DOCUMENT_LINK_TABLE_ID/VIEW_ID` secrets directly.
