...
- after exporting the alternate Feishu values in the current shell, run [`../scripts/configure_hello_docs_binding.sh --dry-run`](../scripts/configure_hello_docs_binding.sh) first, then run it without `--dry-run`; add `--include-optional` when the same shell also has mirror-only Vercel, DingTalk, Feishu IM, Cloudflare tunnel, or OpenClaw adapter variables, and add `--unpause` only when the audit should also flip `FEISHU_BUILD_QUEUE_PAUSED=false`
