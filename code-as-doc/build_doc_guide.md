...
- `queue-query`: OpenClaw Phase 2 queue resolution helper; it reads the Feishu-bound `Review Init` / `Document_link` rows and returns the concrete `record_id`, workflow intent, `Git_ref`, `Document link`, and status fields that a natural-language control layer needs before dispatch; for Publish rows, include `HTML_link` too when that field exists
