...
- `node integrations/openclaw/auto-manual-control-layer/cli.mjs dispatch ...`: local OpenClaw Phase 2 control CLI for `start-review`, `build-draft`, and `publish`; it dispatches the existing `main`-owned GitHub workflows and keeps one lightweight local state file for later `status` lookups
