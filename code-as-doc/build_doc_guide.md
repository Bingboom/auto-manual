...
- after exporting the alternate Feishu values in the current shell, run [`../scripts/configure_hello_docs_binding.sh --dry-run`](../scripts/configure_hello_docs_binding.sh) first, then run it without `--dry-run`; add `--unpause` only when the audit should also flip `FEISHU_BUILD_QUEUE_PAUSED=false`
- run [`../scripts/audit_hello_docs_binding.sh --report-only`](../scripts/audit_hello_docs_binding.sh) to check source/mirror tree parity, the source sync token, mirror variables, and required mirror secret names without exposing secret values
