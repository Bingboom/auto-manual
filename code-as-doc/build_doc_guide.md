...
- use `block_type=table_row` for the normal symbol/meaning grid, `block_type=signal_row` for the warning/caution/note/tip table at the top of the symbols page, and `block_type=alert_label_row` for rewrite-only labels such as `DANGER`
- signal rows must include the four `symbol_key` values `warning`, `caution`, `note`, and `tips`; add `label_*` / `aliases_*` fields when Word/HTML rewrite must recognize localized warning words or singular/plural tip variants. Put labels that should not render in the Symbols-page table, such as `danger`, in `alert_label_row`. These rows own the visible signal token and alert-label aliases, so do not duplicate signal-word content in `Localized_Copy.csv` or Python constants
