...
- `process-review-start-queue`: Start Review bridge; it consumes `sync.phase2.review_init` rows where `是否进入Review` is checked and `Workflow_action` maps to `Start Review`, resolves the review target from `Document_Key` alone, uses explicit `Build_family` when present and otherwise selects the config whose declared `build.targets` / default target matches that model and region, then uses `Lang` only to choose the matching whole-book or single-language surface. This lets `JBP-2000B_US` enter review through `config.bp-us.yaml` without filling `Build_family`, while Draft/Publish keep the explicit-family guard for BP. The worker groups only the rows whose resolved config enables `build.queue_by_document_key`, syncs the latest phase2 snapshot, always reseeds `docs/_review` from the latest `origin/main` template/data state, force-updates the routed review branch when it already exists, creates or reuses the PR, then writes back the same `Git_ref`, `PR_url`, `Review_status=InReview`, and cleared `是否进入Review` state to every pending row in that group
...
- Draft/Publish queue routing uses `Build_family` as the primary selector: `us-merged`, `eu-merged`, `us-en`, `eu-en`, `us-es`, `us-fr`, `pt-br`, `jp-ja`, `cn-zh`, and opt-in families such as `bp-us`; `Lang` is only a compatibility fallback. Start Review may leave `Build_family` blank because its model/region target selects the declaring config.
...
- merged US/EU Draft/Publish rows should use `Build_family = us-merged` / `eu-merged` and may leave `Lang` blank; single-language Draft/Publish rows should use the matching family such as `us-en` / `eu-en` / `us-fr` / `us-es` / `pt-br`. Start Review rows can leave `Build_family` blank when `Document_Key` identifies a uniquely declared target.
