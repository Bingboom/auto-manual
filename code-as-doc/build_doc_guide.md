...
- [`../.github/workflows/feishu-build-queue.yml`](../.github/workflows/feishu-build-queue.yml): `main`-owned GitHub-hosted queue worker for the remote repo; it runs on a 5-minute schedule plus `workflow_dispatch`, bootstraps `lark-cli` with `FEISHU_APP_ID/FEISHU_APP_SECRET`, sets `FEISHU_PHASE2_IDENTITY=bot`, refreshes phase2 from the queue row's resolved family config before each Draft/Publish group, and then consumes the `Document_link` queue
...
- before enabling that remote worker, make sure the Feishu app/bot has read access to the phase2 source tables and write access to the `Document_link` table; literal config bindings can point `spec_master` and `Document_link` at the online tables, while the remaining phase2 table ids can still come from env vars
