...
- [`../.github/workflows/feishu-start-review.yml`](../.github/workflows/feishu-start-review.yml): GitHub-hosted review-init worker for the remote repo; it consumes the review-init table, creates or reuses the review branch, seeds `docs/_review`, pushes the branch, and writes back `Git_ref` plus `PR_url`. It prefers `FEISHU_PHASE2_REVIEW_INIT_TABLE_ID/VIEW_ID`, but can fall back to the `Document_link` table/view secrets when Review Init and Document_link share the same binding.
