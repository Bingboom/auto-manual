...
- `python tools/manifest_family.py fold --root . --index docs/manifests/family/index.yaml` checks the family index: seven anchor YAML manifests plus 31 carrier diffs cover all 38 current YAML goldens with canonical byte identity. `BP@INTL`, `BP@JP`, `Solar@INTL`, `Charger@INTL`, and `MAIN@INTL` keep structurally independent anchors instead of pretending to be diffs of one universal manual. Add `--write` only to refresh the tracked diff carriers; it never edits a YAML manifest.
- `tools/skeleton_resolve.py` keeps the public `emit` / `verify` / `plan` CLI surface unchanged. Its Python `resolve_plan(..., product_plan=...)` API accepts target-owned `house_style_version`, `enabled_optional_slots`, and `terminal_slots` selections; when a blueprint declares order profiles, load and pass both carrier maps with `load_slot_template_catalog(...)` so a version cannot silently lose its safety/warranty variant. Blueprints declare the complete slot universe and named order profiles; optional front/body slots are opt-in, back slots have the single `terminal_slots` selector, and calls without a product plan still resolve the required/capability core. A slot-bearing carrier may additionally declare a safe basename-only `materialized_name`: `slot_id` remains the semantic identity while an already-shipped target preserves its review page filename. Names containing POSIX/Windows path syntax, non-`.rst` names, `pNN_` positional aliases, or collisions fail closed. Entries without the field keep the existing `<slot_id>.rst` behavior, and legacy entries without `slot_id` are unchanged. Do not put a model/title/file/page conditional in the resolver; targets supply only plan/config/source/asset data.
