...
- Existing path-based template images remain compatible and are copied plus recorded as `legacy-path`. They are accounted for, but they are not registry status/scope gated until their RST reference is deliberately migrated to `asset:<asset_key>`. Do not claim that all existing images are managed merely because the usage manifest contains them. Release manifests do not yet carry this asset lineage; `bundle_manifest.json` is the current bundle-level provenance surface.
