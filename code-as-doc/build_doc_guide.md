...
- After a reviewed live-Base change, run `python build.py sync-data --config configs/config.us.yaml --data-root data/phase2`, verify the downloaded manifest and attachments, then promote that exact reviewed snapshot into `tests/fixtures/phase2` in a PR. Read the Docs never fetches mutable live attachments; this explicit freeze is the reproducibility boundary. `sync-data` invokes the active lark-cli profile. On the maintained local Mac, verify `lark-cli --profile prod whoami --as bot` identifies the HT-Docs bot, temporarily select that `prod` profile for the live freeze, and restore the previous profile afterward; `FEISHU_PHASE2_IDENTITY=bot` alone does not select the business application.
