...
- when review-init reuses the shared `Document_link` binding, the start-review worker skips rows whose `Workflow_action` is `Build Draft Package` or `Publish`; only the pending row entering review must map to `Start Review` / `Seed Draft`
