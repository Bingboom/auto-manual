...
- [`../integrations/openclaw/feishu-im-webhook-adapter/`](../integrations/openclaw/feishu-im-webhook-adapter): standalone Feishu IM ingress adapter; it validates callback payloads, normalizes text messages, uses `queue-resolve-action|queue-query|queue-execute` as the repo-owned action surface, and replies back into the same Feishu thread. Typed cloud-doc backport messages such as `cloud-doc backport <Feishu cloud-doc URL> docs/_review/<model>/<region>/page/<page>.rst` are routed to `tools/cloud_doc_backport.py run-review` only for senders in `FEISHU_IM_CLOUD_DOC_BACKPORT_ALLOWED_SENDERS`; they default to dry-run reports, and explicit `--write` requires `FEISHU_IM_CLOUD_DOC_BACKPORT_ALLOW_WRITE=true`.
