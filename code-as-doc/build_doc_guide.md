...
- [`../tools/dingtalk/openapi_upload_cli.py`](../tools/dingtalk/openapi_upload_cli.py) is the first official upload smoke helper for the future `dingtalk_openapi` sink; it expects `DINGTALK_OPERATOR_UNION_ID` and `DINGTALK_DEFAULT_TARGET_NODE_URL`, then runs `uploadInfos/query -> signed upload -> commit` without browser-session values.
- `python build.py dingtalk-control-config --config config.us.yaml --operator-union-id union_xxx --target-node-url https://alidocs.dingtalk.com/i/nodes/... --json` is the repo-owned Feishu control-plane command for the long-lived `dingtalk_openapi` worker path; it validates/parses the target URL, then writes `operator_union_id` plus `default_target_node_url` into the bound Feishu control row.
...
- `process-build-queue`: Build Draft Package / Publish bridge; it consumes `sync.phase2.document_link` rows where `是否触发文档构建 = Y`, write `开始构建时间` immediately when one row is picked up, resolve the matching config family from `Build_family` first and `Lang` second, group only the rows whose resolved config enables `build.queue_by_document_key`, refresh `data/phase2` only when `Document_link.是否强制刷新数据 = true`, run `check + word`, publish the generated DOCX through the configured artifact sink, write the local DOCX path into `Document directory`, write the resolved link into `Document link`, optionally dual-write the DingTalk node URL into `Document link_dd` when that field exists and the sink is DingTalk, write a timestamped build status into `构建结果`, write the refresh result into `data_sync`, clear `是否强制刷新数据`, and flip the trigger back to `已构建` on success
- if the global artifact sink is DingTalk and the row also has `是否上传钉钉`, that checkbox becomes the row-level gate: checked rows use DingTalk and write `Document link_dd`, unchecked rows fall back to the normal Feishu/wiki upload path for that run
- if that checked row also has `DingTalk_target_node_url`, the worker uploads to that row-level target first; otherwise the `dingtalk_openapi` path falls back to the Feishu control-row `default_target_node_url`, while the legacy session path still falls back to `DINGTALK_DOCS_TARGET_NODE_URL`
...
- start that ingress with [`../scripts/run_feishu_im_webhook_adapter.sh`](../scripts/run_feishu_im_webhook_adapter.sh) or [`../scripts/run_feishu_im_webhook_adapter.ps1`](../scripts/run_feishu_im_webhook_adapter.ps1), expose it over HTTPS, then configure the Feishu app event callback URL to `/feishu/events`, keep callbacks in plain mode, subscribe to `im.message.receive_v1`, and publish the app change
- for the smallest always-on deploy, use [`../integrations/openclaw/feishu-im-webhook-adapter/deploy/README.md`](../integrations/openclaw/feishu-im-webhook-adapter/deploy/README.md) together with the bundled `cloudflared` config template, launchd plist templates, and `env.sh` template
...
- the control layer is no longer at the old Phase 0 plan baseline; the repo-local Phase 2 stack is now in place, including queue resolution, deterministic execution, structured failure replies, explicit Publish confirmation, and the standalone Feishu IM ingress adapter. The remaining work is deployment hardening, shared state, and encrypted callback support.
...
- [`../scripts/process_build_queue.ps1`](../scripts/process_build_queue.ps1): Windows automation wrapper for `process-build-queue`; it restores the local Node/npm path plus the `FEISHU_PHASE2_*` user env vars, and when the optional DingTalk sink is enabled it also restores `AUTO_MANUAL_ARTIFACT_SINK_PROVIDER` plus `DINGTALK_DOCS_*`, runs with `--staging-root .tmp/staging`, forwards any extra queue args such as `--dry-run` or `--record-id`, and writes run logs into [`../.tmp/process-build-queue/`](../.tmp/process-build-queue)
- [`../scripts/process_build_queue_feishu.ps1`](../scripts/process_build_queue_feishu.ps1): one-click Windows wrapper that forces `AUTO_MANUAL_ARTIFACT_SINK_PROVIDER=lark_drive` before calling the shared queue wrapper
- [`../scripts/process_build_queue_dingtalk.ps1`](../scripts/process_build_queue_dingtalk.ps1): one-click Windows wrapper that forces `AUTO_MANUAL_ARTIFACT_SINK_PROVIDER=dingtalk_alidocs_session` before calling the shared queue wrapper
...
- if you switch the queue artifact sink to DingTalk, also set `AUTO_MANUAL_ARTIFACT_SINK_PROVIDER=dingtalk_alidocs_session` plus `DINGTALK_DOCS_TARGET_NODE_URL`, `DINGTALK_DOCS_A_TOKEN`, `DINGTALK_DOCS_XSRF_TOKEN`, and `DINGTALK_DOCS_COOKIE` with optional `DINGTALK_DOCS_BX_V`; Feishu envs still stay mandatory because the queue source and writeback table are still Feishu-owned
- for the long-lived `dingtalk_openapi` worker path, keep app credentials in GitHub Secrets and bind one Feishu control row through `FEISHU_PHASE2_DINGTALK_CONTROL_TABLE_ID`, `FEISHU_PHASE2_DINGTALK_CONTROL_VIEW_ID`, and optional `FEISHU_PHASE2_DINGTALK_CONTROL_RECORD_ID`; OpenClaw should update that row through `build.py dingtalk-control-config`, and `process-build-queue` now reads that row on each run instead of depending on browser-session headers
- for the GitHub-hosted `dingtalk_openapi` path, also set repository secret `AUTO_MANUAL_ARTIFACT_SINK_PROVIDER=dingtalk_openapi`; the draft/publish queue workflows now inject `AUTO_MANUAL_ARTIFACT_SINK_PROVIDER`, `DINGTALK_CLIENT_ID`, `DINGTALK_CLIENT_SECRET`, `DINGTALK_CORP_ID`, and `FEISHU_PHASE2_DINGTALK_CONTROL_*` before `process-build-queue`
