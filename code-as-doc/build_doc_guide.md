...
- `HB-SPECIAL-INBOX` keeps the serialized `three-card-responsive` contract unchanged and adds `responsive-card-grid` for a non-empty ordered card list. The latter is Web-only: it auto-fits on desktop, uses three columns on tablets and one column on phones. Its registry bindings mark LaTeX, IDML and Word `not-applicable`, and those adapters reject the variant until fixed-page layouts are implemented.
