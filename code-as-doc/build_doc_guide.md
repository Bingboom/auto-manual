...
- the web profile omits `cover*`, `00_toc*`, and `99_back_cover*`, removes print page breaks, and starts the manual at `00_preface`, whose first visible block is `IMPORTANT`; catalog links point directly to that generated manual entry instead of an intermediate target index
- For targets explicitly listed in the web figure contract (currently `JE-1000F / US`), Product Overview annotations reuse the page's RST image plus its table cells, while supported operation panels reuse the RST image plus governed semantic steps from the adjacent `line-block`. The web projection turns that same content into searchable HTML callouts with SVG leader lines: wide screens use labels over/around the image, and narrow screens use the image followed by an ordered label list. An unlisted target keeps its ordinary source HTML instead of being forced through another product's geometry. Do not duplicate translated labels or operation copy in the web renderer.
