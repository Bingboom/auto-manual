...
- `tools/cloud_doc_backport.py diff`: P0 Feishu cloud-doc backport prototype. It reads a real Feishu doc through `lark-cli docs +fetch` or a local markdown fixture, compares it with a baseline, writes `cloud_doc_backport_report.json` and `.md`, and does not edit templates, `_review`, generated output, or source bitable rows.
