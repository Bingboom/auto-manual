...
- `lcd_icons`, `troubleshooting`, `symbols_blocks`, `symbols_page_copy`, `page_copy`, `variable_defaults`, and `variable_lang_overrides` may be synced as normal phase2 tables; the LCD icons renderer reads `lcd_icons_blocks.csv` and renders downloaded `figure` attachments from `data/phase2/_attachments/lcd_icons/`, the troubleshooting renderer reads `troubleshooting_blocks.csv`, the symbols renderer uses downloaded `Figure` attachments from `data/phase2/_attachments/symbols` when present, `symbols_blocks` can also maintain the signal-word table with `block_type=signal_row`, synced `symbols_page_copy.csv` owns symbols headers and signal labels as a key-plus-language wide table, `page_copy.csv` owns remaining shared page copy with fixed fields `page_id`, `lang`, `copy_key`, `text`, `enabled`, and `order`, and LCD variables resolve from `Variable_Defaults.csv` plus `Variable_Lang_Overrides.csv`
...
- the automatic phase2 default requires a complete manifest-backed core snapshot: `spec_master`, `spec_footnotes`, `spec_notes`, `spec_titles`, `page_copy`, `symbols_page_copy`, `symbols_blocks`, and `troubleshooting` must all appear as requested/synced tables in `snapshot_manifest.json`, and the derived `row_key_mapping` file must be recorded as well; partial `sync-data --table ...` runs are allowed, but they are treated as explicit experiment snapshots unless you pass them through `--data-root`
...
- signal rows must include the four `symbol_key` values `WARNING`, `CAUTION`, `NOTE`, and `TIPS`; keep symbols table headers and signal labels in `symbols_page_copy` with `copy_key` as the first column and language columns after it, while symbols page title and image alt text stay in `page_copy`
