...
手工登记行的 `asset_key` 列会与插图资产表交叉核对，**核对跑在
`build.py asset-check`（已是 CI 作业）里，不在这条月度流程里**：获批的
`二维码` 资产必须登记它编码的内容，隔离或临时候选不得登记为现行印刷目标，
登记行也不得指向未注册资产。二维码目标未必是网址——JE-1000F US 封底 QR
解码为文档物料号 `160102000404`，非 http 目标自动跳过探活。
