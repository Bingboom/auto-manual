...
2. Data layer
   ...
   - GitHub `Manual Validation` uses the committed fixture snapshot under [`../tests/fixtures/phase2`](../tests/fixtures/phase2) so CI stays deterministic after `data/phase2` became gitignored; local live builds should still sync into `data/phase2`.
   ...
   - When the alternate Feishu values are exported locally, run `scripts/configure_hello_docs_binding.sh --dry-run`, then run it without `--dry-run` to write the values into `Hello-Docs`; add `--unpause` only after the audit should allow Feishu runtime workflows to run.
   - Before unpausing `Hello-Docs`, run `scripts/audit_hello_docs_binding.sh --report-only`; it reports source/mirror tree parity, missing GitHub Secret names, and mirror variables without printing secret values.
