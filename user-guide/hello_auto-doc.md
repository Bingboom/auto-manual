...
- RTD itself has no Feishu credentials. After live review, promote the verified manifest and its exact content-addressed attachments into [`tests/fixtures/phase2`](../tests/fixtures/phase2) through a reviewed PR; `.readthedocs.yaml` consumes that frozen snapshot. Do not add Base download calls to the RTD job or restore static image paths in `web_manual.json`.
