...
2. Data layer
 ...
 - for a fixed "现在库里构建了多少文档" lookup, use explicit filters instead of broad free text: run `python build.py queue-query --config config.us.yaml --queue-scope document-link --result-contains success --limit 200 --json` and the same command with `config.ja.yaml`, then count rows whose `normalized_workflow_action` is `draft` or `publish`; this is the stable operator path when one market like `JP` must not be missed
