...
- Queue Publish also freezes the complete manifest-backed phase2 root under `reports/releases/<model>/<region>/<lang>/versions/<version>/snapshot/`. The release JSON/CSV points to this archive instead of the mutable live root and records `snapshot_sha256`, source-manifest revision, freeze time, and target matrix. Keep the archive for the full release lifetime; same-version retries may reuse byte-identical data, but changing the binding or editing archived bytes fails closed. Rebuild equivalence from this archive is the next E1 slice and is not claimed by the freeze step alone.
