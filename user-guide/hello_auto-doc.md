...
2. Data layer
   ...
   - For first-pass intake from a structured spec/manual Markdown or Feishu cloud doc, run `python tools/source_intake.py run --input <spec.md-or-doc-url> --document-key <MODEL_REGION> --source-lang en --data-root data/phase2 --out reports/source_intake/<run-id>`. This produces reviewable source-table candidates and existing-row change requests; it does not create new online rows or bypass the human-approved source-table writer. Use [`../code-as-doc/dev/source_intake_mvp_checklist.md`](../code-as-doc/dev/source_intake_mvp_checklist.md) as the staged checklist.
