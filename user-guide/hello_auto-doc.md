...
2. Data layer
 ...
 - **Reseed drops hand-curated print-only pages.** The seeded index is generated from the page manifest, so review-index includes that were added by hand and have no manifest entry (e.g. the JE-1000F/US print book's `00_toc.rst` and `99_back_cover.rst`) silently lose their references on every reseed — the page files stay in `page/` but leave the built book, and the target then fails the same-source IDML gate at Publish (incident: 2026-08-13 reseed → run 31767694706). Until seeding preserves curated includes or the manifest can declare print-only pages, re-add those includes to the review index after every reseed of a print-published target and re-verify against the approved reference-layout contract
