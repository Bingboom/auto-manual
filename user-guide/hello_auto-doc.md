...
- Manifest pages can carry a `capability:` key (the capability field name): at bundle-plan time the assembler keeps or drops the entry per target using the same capability mirror, so one family manifest declares the page superset and each model auto-selects its chapters (`manual_us.yaml`: `06_ups_mode` ↔ `UPS功能`, `07_extra_battery` ↔ `加电包扩容`). Targets without a capability row keep every page.
