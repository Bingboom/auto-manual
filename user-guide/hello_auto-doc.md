...
2. Data layer
 ...
 - builds report results on an accept-first lifecycle, never by holding the chat turn open: the dispatch reply and `/manual-status` carry `state: accepted|processing|completed|failed` plus a `note:` pointing back to `status last`, so an in-flight run reads as `任务正在处理中` (not a failure). On the Feishu IM adapter a single-record build now mirrors the batch flow — it replies "已受理（处理中）" immediately, dispatches with `--no-wait`, then a background poll sends a follow-up (`已完成` once the Base row is fresh, or `任务正在处理中` if it is still building when the poll window ends); either way you can re-ask "这个好了没" to re-query on demand
