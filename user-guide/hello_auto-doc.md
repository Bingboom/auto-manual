...
2. Data layer
   ...
   - `Bingboom/auto-manual` is the code source of truth. Its `sync-hello-docs.yml` workflow mirrors the exact `main` Git tree one-way into `Bingboom/Hello-Docs`, without re-adding a checked-out tree that could rewrite file blobs through line-ending attributes; configure `HELLO_DOCS_SYNC_TOKEN` only in the source repo, keep `Hello-Docs` Feishu/OpenClaw bindings in that repo's own GitHub Secrets / Variables, and leave `FEISHU_BUILD_QUEUE_PAUSED=true` in the mirror repo until those bindings are ready. That pause variable is scoped to mirror Feishu runtime workflows and does not pause source repo behavior.
 ...
 - batch document-link replies in Feishu IM are sent as one status summary plus one message per `Document link`; short follow-ups such as `发` or `发一下` reuse the previous batch context and resend those document links instead of flattening them into one plain-text block
...
- `python build.py process-build-queue --config configs/config.us.yaml` is the optional Feishu task-table bridge: it reads `sync.phase2.document_link` rows where `是否触发文档构建 = Y`, first writes and verifies a two-hour `claim_token` lease in `构建结果`, writes `开始构建时间` when that field exists, resolves the matching config family from `Build_family` first and `Lang` second, groups only the rows whose resolved config enables `build.queue_by_document_key`, runs `sync-data` only when that row group has `是否强制刷新数据 = true`, builds Draft rows as `check -> word -> md`, upgrades Publish rows to `check -> diff-report -> word -> pdf -> md`, uploads the Draft DOCX or Publish PDF to the primary Feishu/wiki destination, can also sync that same primary artifact to DingTalk as an optional mirror, writes the local DOCX release path into `Document directory`, keeps `Document link` as the DOCX link for Draft and PDF link for Publish, imports the built Word `.docx` (images embedded) into `飞书云文档` when that field exists — the Markdown export cannot be used for the cloud doc because Feishu can't resolve its local image paths, optionally writes the mirrored DingTalk node URL into `Document link_dd`, writes a timestamped status into `构建结果`, writes the refresh result into `data_sync`, clears `是否强制刷新数据`, and flips the trigger back to `已构建` on success
...
- config policy for `build.queue_by_document_key`: enable it for merged whole-book families that intentionally produce one shared manual across multiple languages, such as today's `us-merged`, `eu-merged`, and future `cn-merged`; keep it disabled for single-language families such as `us-en`, `eu-en`, `us-fr`, `us-es`, `pt-br`, `jp-ja`, `cn-zh`, or future `eu-de` / `eu-fr`, which should continue to run one queue row per `record_id`
   - Publish queue DOCX/PDF/Markdown outputs are staged under [`../reports/releases/<model>/<region>/<lang>/versions/<version>/`](../reports/releases), Markdown sidecars such as `assets/`, `conf.py`, and `index.md` are preserved when present, and each latest publish HTML snapshot is mirrored under [`../reports/releases/<model>/<region>/<lang>/latest/html/`](../reports/releases) for Vercel hosting. The deployed site is isolated under `dist/<model>/<region>/<lang>/` and linked from a root index; if the `Document_link` table has an `HTML_link` field, the remote publish workflow writes each target URL back to its queue row, while the unmasked raw URL is also preserved in `publish_meta.json` and `openclaw-run-metadata`. `--single-target` remains available for legacy hosting.
  ...
  - [`../.github/workflows/feishu-build-queue.yml`](../.github/workflows/feishu-build-queue.yml) is the `main`-owned remote-repo Publish worker: OpenClaw / natural-language control calls its `workflow_dispatch` API on `main` with the target `record_id`, so it does not depend on a local always-on machine. The former 5-minute schedule is preserved as a commented block in the workflow and should stay disabled unless unattended polling is deliberately re-enabled after bot secrets and Feishu permissions are healthy.
   ...
   - if you want remote immediate builds, create a Feishu workflow whose combined condition is `是否触发文档构建 = Y` and `是否立即构建 = true`, then call the GitHub `workflow_dispatch` API for `feishu-build-queue.yml` on `main`; the queue still only builds rows whose trigger field is `Y`
...
- queue-driven `Workflow_action=Publish` additionally stages the formal DOCX, PDF, and Markdown under [`../reports/releases/<model>/<region>/<lang>/versions/<version>/`](../reports/releases), uploads the PDF URL back to `Document link`, imports the built Word `.docx` (images embedded) into `飞书云文档` when that field exists, mirrors each target's newest publish HTML under [`../reports/releases/<model>/<region>/<lang>/latest/html/`](../reports/releases) for Vercel, and writes the per-target deployed Vercel URL back to `HTML_link` when that field exists
...
- the web profile skips `cover*`, `00_toc*`, and `99_back_cover*` and opens the manual directly at the `IMPORTANT` content in `00_preface`; the merged-language inventory line is hidden only in this web projection. The catalog links to that manual entry rather than making readers pass through a nested target index
- For targets listed in [`web_manual.json`](../docs/renderers/contracts/web_manual.json) (currently `JE-1000F / US`), both Product Overview views use centered locale-matched approved PDF artwork in English, French, and Spanish, with the searchable HTML labels retained as the narrow-screen semantic fallback. The image crop excludes the FRONT/RIGHT view heading so theme changes still control it. WHAT'S IN THE BOX is rebuilt from its source cells as three equal rounded cards with 1/2/3 badges and a full-width TIP strip, matching the PDF hierarchy while keeping the localized copy live. App Setup renders sharp store-badge and QR artwork as two equal columns, centers each image within its own column, keeps the matching two descriptions as live responsive HTML, and replaces the duplicate visible step-2.1 add-device wording with the themeable plus while preserving the localized label for screen readers. Operation figures, the car-charging connection panel, and the App add-device phone/button panel use centered, proportional localized 2x crops of the approved PDF content block. The crop never contains the section heading, so theme changes still control headings, typography, colors, and spacing; the App's 2.3 step also remains live HTML outside the screenshot. In the web profile, every ordinary standalone RST image fills the same responsive content width, remains centered, and preserves its aspect ratio instead of retaining inconsistent print-width hints such as 360 px. The original RST image and governed adjacent `line-block` stay in the hidden semantic layer. Unlisted targets retain ordinary source HTML until their own presentation is validated and added to the contract.
- FCC is rendered from the localized RST as a searchable two-column card with the FCC mark and becomes one column on phones. Each localized MEANING OF SYMBOLS warning-definition table vertically centers its signal-name cells beside multi-line explanations. The following safety-symbol matrix is rendered from the same localized RST as two independent rounded Symbol/Meaning tables, matching the PDF's left-six/right-five structure so a long right-side description does not stretch the paired left row. On desktop both panels share the same outer height and aligned top/bottom borders; phones stack the two tables. The LCD icon page remains a searchable four-column HTML table: `On` / `Blink` / `Off` line-blocks stay on separate lines; the rounded frame and every row/column rule mirror the PDF hierarchy; number/icon/name cells are lightly filled; compact number badges are centered in the first column; and phones use horizontal scrolling instead of crushing the copy.
