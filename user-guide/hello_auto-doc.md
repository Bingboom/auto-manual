...
2. Data layer
 ...
 - OpenClaw can also resolve explicit batch Draft asks such as `输出JE-1000F的所有欧规说明书文案`; it maps `欧规` into a `Task_id` prefix like `JE-1000F_EU_`, keeps only `Build Draft Package` rows with `是否触发文档构建` enabled, and dispatches those rows by Feishu `record_id`. `是否强制刷新数据` is still honored by the build script as the row-level refresh input.
