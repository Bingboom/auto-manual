...
2. Data layer
   ...
   - `Bingboom/auto-manual` is the code source of truth. Its `sync-hello-docs.yml` workflow mirrors `main` one-way into `Bingboom/Hello-Docs`; configure `HELLO_DOCS_SYNC_TOKEN` only in the source repo, keep `Hello-Docs` Feishu/OpenClaw bindings in that repo's own GitHub Secrets / Variables, and leave `FEISHU_BUILD_QUEUE_PAUSED=true` in the mirror repo until those bindings are ready. That pause variable is scoped to the mirror queue job and does not pause the source repo queue.
