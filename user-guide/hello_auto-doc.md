...
- config policy for `build.queue_by_document_key`: enable it for merged whole-book families that intentionally produce one shared manual across multiple languages, such as today's `us-merged`, `eu-merged`, and future `cn-merged`; keep it disabled for single-language families such as `us-en`, `eu-en`, `us-fr`, `us-es`, `pt-br`, `jp-ja`, `cn-zh`, or future `eu-de` / `eu-fr`, which should continue to run one queue row per `record_id`
   - print Publish stages DOCX/PDF/Markdown under [`../reports/releases/<model>/<region>/<lang>/versions/<version>/`](../reports/releases) and does not deploy HTML. Web Publish always refreshes approved Web assets, writes MyST plus verification HTML under `versions/<version>/web/`, freezes the aggregate catalog in `Hello-Docs/publish:docs/publish/`, and writes the deterministic RTD route to `HTML_link`.
...
- Read the Docs renders the frozen Web Publish catalog from `Hello-Docs/publish:docs/publish/web/`; it does not replace review-preview packaging or formal print release outputs
...
- queue-driven `Workflow_action=Web Publish` forces live asset sync, renders web-profile MyST/HTML, advances `Hello-Docs/publish:docs/publish/`, and writes RTD `HTML_link`; it does not upload or overwrite IDML/PDF/DOCX outputs
...
- RTD builds the frozen `docs/publish/web/` catalog from the `Hello-Docs/publish` branch. The review/fixture command in [`.readthedocs.yaml`](../.readthedocs.yaml) is only the bootstrap fallback before the first snapshot.
...
- Web Publish first materializes target-scoped `md` directories, then assembles `docs/publish/web/` as the homepage catalog without rewriting the repo-root [`docs/index.rst`](../docs/index.rst)
...
- `feishu-web-publish-queue.yml` is the Web Publish worker on `Hello-Docs/main`; it writes only the generated `publish` branch and RTD link.
