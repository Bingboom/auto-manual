...
- if publish queue rows should expose the same stable public entry URL, add `RTD_link` to `Document_link` and set the GitHub repository variable `AUTO_MANUAL_RTD_URL`
...
- for rapid sibling-language `build-draft` bursts, the OpenClaw bridge now reuses one recent shared draft-queue worker instead of dispatching one GitHub queue run per language, so `en + es + fr` batch requests do not cancel each other at the Actions concurrency gate before the job starts.
