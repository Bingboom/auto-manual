...
2. Data layer
 ...
 - the same adapter can now accept typed Feishu cloud-doc review backport asks such as `cloud-doc backport <飞书云文档链接> docs/_review/<model>/<region>/page/<page>.rst`; if the message or cloud-doc title includes a target like `manual_je2000f_eu_en_0.7`, the `docs/_review/...rst` path can be omitted when the adapter finds one safe review source candidate. This path is gated by `FEISHU_IM_CLOUD_DOC_BACKPORT_ALLOWED_SENDERS`, defaults to dry-run reports, and only honors explicit `--write` when `FEISHU_IM_CLOUD_DOC_BACKPORT_ALLOW_WRITE=true`. It calls `tools/cloud_doc_backport.py run-review`, reports `PR_READY` when verification passes, and still does not write Feishu source tables. Data-like deltas get a `cloud_doc_backport_source_table_suggestions.md` operator report with candidate source-table hints. Opening the GitHub PR is a second explicit message, `cloud-doc backport-pr reports/cloud_doc_backport/<run-id>/cloud_doc_backport_run.json`, gated by `FEISHU_IM_CLOUD_DOC_BACKPORT_ALLOW_PR_CREATE=true`.
...
3. Review working layer
   ...
   - Accepted Feishu cloud-doc revisions back-port through `python tools/cloud_doc_backport.py run-review-branch --doc-name <doc name> --cloud-doc <url>` — the blessed path: it resolves the review branch, diffs the cloud-doc against a **render baseline** (so deltas are the reviewer's real edits, not RST-source noise), and with `--write --push` applies only Class R prose and opens a draft PR into the review branch. The older `run-review --doc-url ... --source-path docs/_review/...` diffs against the RST source and is now **guarded**: a `--write` against an `.rst` baseline is refused and steered to `run-review-branch` unless `--allow-rst-baseline` is set (the Feishu IM adapter passes it automatically, scoping to one resolved page). The Feishu IM adapter may infer that source path from a cloud-doc title such as `manual_je2000f_eu_en_0.7` when the current review checkout has one safe candidate; ambiguous cases still ask for the exact `docs/_review/...rst`. The runners write diff/apply/run reports and are dry-run by default. Add `--write` only after reviewing the apply report; write mode patches guarded review prose, runs residual verification, and marks the run `PR_READY` only when the review source changed and verification passed. Data-like deltas stay report-only and also get `cloud_doc_backport_source_table_suggestions.md` with candidate source tables and operator steps. Use `python tools/cloud_doc_backport.py open-pr --manifest reports/cloud_doc_backport/<run-id>/cloud_doc_backport_run.json` only after `PR_READY`; it commits the changed `_review` source to a draft PR and leaves local reports out of the commit.
