...
- the RTD catalog leads with an **InDesign edition** per manual: a print-layout page-card web document rendered by [`tools/render_web_edition.py`](../tools/render_web_edition.py) from the same manual IR the IDML export uses, with the flow HTML manual kept as a secondary link. Its text reflows with a web fallback font (print Gilroy is not redistributable), so it is not pixel-identical to the InDesign PDF — the InDesign/PDF delivery stays the pixel-authoritative artifact; the web edition shares the content and page sequence
