...
- Draft/Publish routing uses `Build_family` as the primary selector: `us-merged`, `eu-merged`, `us-en`, `eu-en`, `us-es`, `us-fr`, `pt-br`, `jp-ja`, `cn-zh`, and opt-in families such as `bp-us`; `Lang` is an optional compatibility field. Start Review can leave `Build_family` blank because `Document_Key` identifies the model/region target.
- merged US/EU Draft/Publish rows should use `Build_family = us-merged` / `eu-merged` and may leave `Lang` blank; single-language Draft/Publish rows should use the matching family such as `us-en` / `eu-en` / `us-fr` / `us-es` / `pt-br`. Start Review rows only need a unique declared target.
