...
- For targets listed in [`web_manual.json`](../docs/renderers/contracts/web_manual.json) (currently `JE-1000F / US`), both Product Overview views use centered locale-matched approved PDF artwork in English, French, and Spanish, with the searchable HTML labels retained as the semantic fallback. The image crop excludes the FRONT/RIGHT view heading so theme changes still control it. WHAT'S IN THE BOX is rebuilt from its source cells as three equal rounded cards with 1/2/3 badges and a full-width TIP strip, matching the PDF hierarchy while keeping localized copy live. App Setup renders the store badges and QR as two distinct shared images, centers each in its own column, and keeps both descriptions as live responsive HTML. Step 2.1 uses the themeable plus while preserving its localized screen-reader label. The add-device panel combines shared two-phone artwork and shared text-free device-control art with the three localized RST button labels as visible HTML and shared 2.1/2.2 captions, so it no longer needs English/French/Spanish composite crops. Operation figures and the car-charging connection panel retain centered locale-matched 2x crops; the App connect-result panel uses one shared figure with 2.3/2.4/2.5 HTML captions. Reference artwork never contains the section heading, and surrounding instructions remain live HTML. In the web profile, every ordinary standalone RST image fills the same responsive content width, remains centered, and preserves its aspect ratio instead of retaining inconsistent print-width hints such as 360 px. Unlisted targets retain ordinary source HTML until their own presentation is validated and added to the contract.
