...
2. Data layer
 ...
 - `python build.py queue-query --config config.us.yaml --queue-scope all --document-id JE-1000F_US_0.3 --json` is the recommended local Phase 2 lookup before a natural-language OpenClaw action; it resolves the exact Feishu row and returns the `record_id`, `Workflow_action`, `Git_ref`, `Document link`, and `构建结果`, plus `HTML_link` for Publish rows when available
