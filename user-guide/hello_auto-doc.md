...
python build.py web-sideload --config configs/config.us-en.yaml --model JE-1000F --region US --lang en --version git-2026-09-16-abc123 --md-dir /tmp/converted-md
python build.py web-receipt --config configs/config.us-en.yaml --model JE-1000F --region US --lang en
python build.py web-receipt --config configs/config.us-en.yaml --model JE-1000F --region US --lang en --write
```
...
- an authorized Git-only Web release takes reviewed, committed sources and `source_manifest.json` through exact-ref checks, Web MyST, strict Sphinx, release metadata, and the same publish assembler. Building, staging and assembling read or write no online tables and create no queue row; only the explicit receipt step below writes any online table, and only after the release is verified. Its durable proof is the source and Hello-Docs commits, hashes, manifests, RTD build commit, canonical routes and aliases; see the [Git-only transaction](../code-as-doc/dev/web_publish_pipeline.md#22-git-only-transaction)
...
- `python build.py web-sideload --config <config> --model <M> --region <R> --lang <L> --version <V> --md-dir <bundle> [--debt "category:location:payoff action"]... [--dry-run]` is the bypass intake for a book that must go live before its structured intake (spec extraction, templating) is done: it stages an externally converted MyST Markdown source (typically transcribed from the shipped PDF with PyMuPDF — never `pdftotext`, which interleaves double-column text) using the same staging/metadata functions `web-release` uses, so `web-assemble` collects it with equal standing. `--md-dir` must already be shaped like a staged `md/` bundle, with a `manual_<stem>.md` filename matching the config's output-naming template — a mismatch fails with the exact expected filename. Inside that bundle you write **prose plus semantic component directives** (`{callout}`, `{spec-table}`, `{troubleshooting}`, `{lcd-icons}`, `{symbols}`, `{lcd-mode}`, `{comparison}`, `{manual-table}`); hand-written component HTML and inline styles are refused, because only the directive layer emits the structure the shared stylesheet needs, and hand-written markup ships an unstyled shell. Before staging, `web-sideload` verifies in two stages — the author plane (markup lint + a strict `sphinx -W -b html` with the directive extension loaded) and then the compiled plane under RTD's own `myst_parser,tools.rtd_portal` extension set — and stages the **compiled** Markdown plus that second build's HTML (there is no separate pipeline HTML build to reuse). Your `--md-dir` is never modified; compilation runs on a copy. Compiling is mandatory because RTD overrides `conf.py` with `-D extensions=...` and can never load the directive layer. Because there was no RST -> Web-profile pipeline run, the staged `publish_meta.json` carries `"source_kind": "pdf_sideload"` and no `language_projection_evidence_*` fields — the only marker that exempts a target from the otherwise-mandatory evidence gate every pipeline-built target still enforces. One `整本未结构化` debt entry is recorded on every run regardless of `--debt`, through the same debt ledger `web-release` uses. `--dry-run` runs only the collision precheck and `--md-dir` shape check. Payoff is mechanical: once real phase2 data exists for the target, an ordinary `web-release` run for the same model/region/lang overwrites the sideloaded metadata in place. See [`pdf-web-sideload`](../.agents/skills/pdf-web-sideload/SKILL.md) for the full workflow and [`web_publish_pipeline.md#23-pdf-sideload-bypass`](../code-as-doc/dev/web_publish_pipeline.md#23-pdf-sideload-bypass) for the contract.
- `python build.py web-assemble` is the "one wave, one command" collating step that runs steps 4-5 of the Git-only transaction (see [`web_publish_pipeline.md#22-git-only-transaction`](../code-as-doc/dev/web_publish_pipeline.md#22-git-only-transaction)): it makes an isolated blobless clone of Hello-Docs (full commit graph, lazy blobs) under a scratch temp directory (never the operator's own local Hello-Docs checkout), reconciles it against the existing `publish` branch tip when one exists (preserving any targets already staged there and refreshing the tracked code/config to current `main`), assembles every book staged under `--releases-root` (default `reports/releases`) with `tools/publish_branch_assembly.py`, and runs the same aggregate `sphinx -W -b html` verification. It then rejects the run if the candidate's three-dot diff against `main` touches anything outside `docs/publish/**` -- the same scope guard `feishu-web-publish-queue.yml`'s "Validate publish PR scope" step enforces. By default it stops there: assemble + strict verify + scope guard only, printing which targets were included, the manifest hash, and the changed-file count, with no network write. Only `--push` advances the shared `publish` branch with an ordinary (never-forced) push and opens or updates the single `publish -> main` PR; a non-fast-forward remote fails instead of being overwritten. Merging that PR always stays a manual, human step, in every path. `--title` and `--hello-docs-repo` (an `OWNER/REPO` slug, a full git URL, or a local path for tests) override the assembler title and remote; `--dry-run` prints the resolved plan and touches no clone, network, or filesystem write at all.
- `python build.py web-receipt --config <config> [--model <M> --region <R> --lang <L>] [--releases-root <root>] [--base-url <url>] [--receipt-record-id <rec> ...] [--write]` runs step 7 of the Git-only transaction — the receipt that `write_web_publish_html_link.py` could never do for a Git-only book, because that book's `publish_meta.json` has no `queue_record_ids` (there was never a queue row). For every `latest/web/publish_meta.json` target under `--releases-root` (default `reports/releases`, narrowed by `--model`/`--region`/`--lang` when given), it locates the `Document_link` row through a priority chain — explicit `--receipt-record-id` (repeatable) first, then `publish_meta.json`'s own `queue_record_ids` if non-empty, then a live search across `Document_link` by model/region/lang — writes `HTML_link`, then creates or updates the matching published-manual catalog (发布文档管理) row keyed by (model, region, lang, `doc_type=web`), and GETs each written record back to confirm the persisted value before reporting success. Zero matching `Document_link` rows is reported as skipped rather than auto-repaired (it never creates a queue row); more than one candidate in either table is rejected with every candidate record id listed, for the operator to resolve by hand. Defaults to dry-run: without `--write` it only reads local `publish_meta.json` files and prints the resolved plan, touching no live table; pass `--write` to perform the live writes. The catalog's `文档类型=web` value and the `区域`/`源语言` labels it writes are new conventions this command introduces (see [`tools/manual_catalog_writeback.py`](../tools/manual_catalog_writeback.py)) — confirm them against the live Base before the first `--write` run for a new region/language.
