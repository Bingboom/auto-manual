...
2. Data layer
   ...
   - When the alternate Feishu values are exported locally, run `scripts/configure_hello_docs_binding.sh --dry-run`, then run it without `--dry-run` to write the values into `Hello-Docs`; add `--include-optional` when the same shell also has mirror-only Feishu IM / OpenClaw adapter values, and add `--unpause` only after the audit should allow Feishu runtime workflows to run.
