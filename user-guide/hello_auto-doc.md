...
2. Data layer
 ...
 - the same adapter can now accept typed Feishu cloud-doc review backport asks such as `cloud-doc backport <飞书云文档链接> docs/_review/<model>/<region>/page/<page>.rst`; this path is gated by `FEISHU_IM_CLOUD_DOC_BACKPORT_ALLOWED_SENDERS`, defaults to dry-run reports, and only honors explicit `--write` when `FEISHU_IM_CLOUD_DOC_BACKPORT_ALLOW_WRITE=true`. It calls `tools/cloud_doc_backport.py run-review`, reports `PR_READY` when verification passes, and still does not write Feishu source tables. Opening the GitHub PR is a second explicit message, `cloud-doc backport-pr reports/cloud_doc_backport/<run-id>/cloud_doc_backport_run.json`, gated by `FEISHU_IM_CLOUD_DOC_BACKPORT_ALLOW_PR_CREATE=true`.
...
3. Review working layer
   ...
   - Accepted Feishu cloud-doc revisions can run through `python tools/cloud_doc_backport.py run-review --doc-url ... --source-path docs/_review/...`; the runner writes diff/apply/run reports and is dry-run by default. Add `--write` only after reviewing the apply report; write mode patches guarded review prose, runs residual verification, and marks the run `PR_READY` only when the review source changed and verification passed. Data-like deltas stay report-only and should be routed back to the Feishu source tables deliberately. Use `python tools/cloud_doc_backport.py open-pr --manifest reports/cloud_doc_backport/<run-id>/cloud_doc_backport_run.json` only after `PR_READY`; it commits the changed `_review` source to a draft PR and leaves local reports out of the commit.
