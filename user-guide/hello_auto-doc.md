...
- config policy for `build.queue_by_document_key`: enable it only for merged whole-book families that intentionally produce one shared manual across multiple languages, such as today's `us-merged` and future `eu-merged` / `cn-merged`; keep it disabled for single-language families such as `us-en`, `us-fr`, `us-es`, `jp-ja`, `cn-zh`, or future `eu-de` / `eu-fr`, which should continue to run one queue row per `record_id`
 ...
 - if you switch the artifact sink to DingTalk, Feishu still remains the queue control plane and writeback surface; only the uploaded artifact destination changes, so `Document link` will hold a DingTalk node URL while the rest of the queue fields stay in Feishu
 - for OpenClaw Phase 2, keep `Document link` as the only required artifact link field; do not introduce `Document link_dd` unless you are explicitly implementing the later DingTalk dual-write extension
