...
- `JE-2000E / EU` follows the same shape from [`manual_sources/JE-2000E/EU/en/2.0/phase2`](../manual_sources/JE-2000E/EU/en/2.0/phase2). Each fr–uk route binds a two-entry illustration manifest: the App add-device screens (2.1/2.2) and connection result (2.3–2.5), shared panels cut at 12x from the English block's pages 23–24 with quarantined App recipes. The add-device figure of en/fr/es/de/it is instead each language block's own crop of the screens plus the control-panel box, with the printed button labels; the page's four label lines move into the figure's alt text. uk keeps the shared screens and its live labels, because the Ukrainian print labels the AC2 button `AC1`.
