...
2. Data layer
 ...
 - `queue-execute` treats a `Start Review` row that already has `Review_status=InReview` and `Git_ref` as completed and returns the current row without dispatching another Action; this keeps rapid batch review requests from creating extra single-row failure runs after the shared worker has already updated sibling targets
...
- for rapid sibling-target `start-review` and sibling-language `build-draft` bursts, the OpenClaw bridge now reuses one recent shared queue worker instead of dispatching one GitHub queue run per row, so batch requests do not block on the first matched record or collide at the Actions concurrency gate before the job starts. If a later `start-review` resolution lands on a row that the shared worker already marked `InReview` with `Git_ref`, `queue-execute` returns that completed row instead of launching another workflow.
