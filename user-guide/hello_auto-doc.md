...
2. Data layer
   ...
   - for the planned DingTalk path, [`../tools/dingtalk/spike_cli.py`](../tools/dingtalk/spike_cli.py) is the generic Phase 0 smoke helper; it gets an App-Only token by default, then lets you supply the exact DingTalk list/update/upload endpoints for the chosen product before anything is wired into `build.py`
   ...
   - [`../tools/dingtalk/openapi_upload_cli.py`](../tools/dingtalk/openapi_upload_cli.py) is the first official upload smoke helper for the future `dingtalk_openapi` path; it expects one bound `operator_union_id` plus a target node URL, then runs `uploadInfos/query -> signed upload -> commit` without browser-session headers
   - `python build.py dingtalk-control-config --config config.us.yaml --operator-union-id union_xxx --target-node-url https://alidocs.dingtalk.com/i/nodes/... --json` is the repo-owned Feishu control-plane command for that worker path; OpenClaw should call it to validate/store `operator_union_id` plus the default DingTalk target row
   ...
   - `python build.py process-review-start-queue --config config.us.yaml --data-root .tmp/review-start/phase2` is the Start Review bridge: it reads `sync.phase2.review_init` rows where `是否进入Review` is checked and `Review_status` is empty / `NotStarted`, resolves each row from `Build_family` first and `Lang` second, groups only the rows whose resolved config enables `build.queue_by_document_key`, syncs a fresh phase2 snapshot, creates or reuses one review branch, seeds `docs/_review`, pushes the branch, creates or reuses the PR, then writes the same `Git_ref`, `PR_url`, `Review_status=InReview`, and cleared `是否进入Review` state back to every row in that routed group
 ...
 - the same Feishu IM adapter can now also call `build.py dingtalk-control-config` directly for DingTalk binding asks such as `查看钉钉配置` or `绑定钉钉 union_xxx https://alidocs.dingtalk.com/i/nodes/...`, so the chat entrypoint can update the worker's default `operator_union_id` plus target node without touching GitHub Secrets
 - start that adapter with [`../scripts/run_feishu_im_webhook_adapter.sh`](../scripts/run_feishu_im_webhook_adapter.sh) or [`../scripts/run_feishu_im_webhook_adapter.ps1`](../scripts/run_feishu_im_webhook_adapter.ps1), expose it over HTTPS, point the Feishu app callback URL to `/feishu/events`, keep callbacks in plain mode, subscribe `im.message.receive_v1`, and publish the app change before testing chat commands
 - if you want the smallest always-on deploy on macOS, use [`../integrations/openclaw/feishu-im-webhook-adapter/deploy/README.md`](../integrations/openclaw/feishu-im-webhook-adapter/deploy/README.md); it includes a `cloudflared` tunnel template, an adapter env template, and launchd plist templates
 ...
 - against the Feishu message control plan, the repo now has the full repo-local Phase 2 stack: query, deterministic execute, structured failure replies, explicit Publish confirmation, and a standalone Feishu IM webhook adapter are all live. The remaining gaps are deployment hardening, shared state, and encrypted callback support.
 ...
 - if OpenClaw dispatches one explicit review-start `record_id` but the GitHub worker cannot re-read that row as pending from the current Feishu view, that run now also emits a structured failure summary instead of ending as a silent success
 - `python build.py process-build-queue --config config.us.yaml` is the optional Feishu task-table bridge: it reads `sync.phase2.document_link` rows where `是否触发文档构建 = Y`, writes `开始构建时间` as soon as one row is picked up, resolves the matching config family from `Build_family` first and `Lang` second, groups only the rows whose resolved config enables `build.queue_by_document_key`, runs `sync-data` only when that row group has `是否强制刷新数据 = true`, then builds the local Word file, publishes that artifact through the configured sink, writes the local path into `Document directory`, writes the resolved link into `Document link`, optionally dual-writes the DingTalk node URL into `Document link_dd` when that field exists and the sink is DingTalk, writes a timestamped status into `构建结果`, writes the refresh result into `data_sync`, clears `是否强制刷新数据`, and flips the trigger back to `已构建` on success
   - for the GitHub-hosted `dingtalk_openapi` path, set repo secret `AUTO_MANUAL_ARTIFACT_SINK_PROVIDER=dingtalk_openapi` and add `DINGTALK_CLIENT_ID`, `DINGTALK_CLIENT_SECRET`, `DINGTALK_CORP_ID`, `FEISHU_PHASE2_DINGTALK_CONTROL_TABLE_ID`, and `FEISHU_PHASE2_DINGTALK_CONTROL_VIEW_ID`; the queue workflow now exports those directly to the runner
...
- config policy for `build.queue_by_document_key`: enable it only for merged whole-book families that intentionally produce one shared manual across multiple languages, such as today's `us-merged` and future `eu-merged` / `cn-merged`; keep it disabled for single-language families such as `us-en`, `us-fr`, `us-es`, `jp-ja`, `cn-zh`, or future `eu-de` / `eu-fr`, which should continue to run one queue row per `record_id`
   ...
   - [`../scripts/process_build_queue.ps1`](../scripts/process_build_queue.ps1) is the Windows automation wrapper for that queue bridge; it restores the local Node/npm path plus the saved `FEISHU_PHASE2_*` user env vars, and if the optional DingTalk sink is enabled it also restores `AUTO_MANUAL_ARTIFACT_SINK_PROVIDER` plus `DINGTALK_DOCS_*`, then writes logs into [`../.tmp/process-build-queue/`](../.tmp/process-build-queue) and forwards extra queue args such as `--dry-run` or `--record-id`
   - [`../scripts/process_build_queue_feishu.ps1`](../scripts/process_build_queue_feishu.ps1) is the one-click Feishu queue entry on Windows; it fixes the upload target to Feishu without requiring you to set `AUTO_MANUAL_ARTIFACT_SINK_PROVIDER` manually first
   - [`../scripts/process_build_queue_dingtalk.ps1`](../scripts/process_build_queue_dingtalk.ps1) is the one-click DingTalk queue entry on Windows; it fixes the upload target to DingTalk while the queue rows, triggers, and writeback table still stay in Feishu
   - for the full current browser-session setup, including how to capture `a-token`, `x-xsrf-token`, and the full cookie string, see [`./dingtalk_alidocs_upload_setup_guide.md`](./dingtalk_alidocs_upload_setup_guide.md)
   - the target long-lived worker shape is now repo-owned end to end: app credentials live in GitHub Secrets, OpenClaw should bind and persist `operator_union_id` plus a default DingTalk target node through `build.py dingtalk-control-config`, and `process-build-queue` reads that Feishu control row on each run
 ...
 - if you switch the artifact sink to DingTalk, Feishu still remains the queue control plane and writeback surface; `Document link` stays the canonical returned link, and if your table also has `Document link_dd` the queue will dual-write the same DingTalk node URL there
 - if the row also has `是否上传钉钉`, that checkbox becomes the row-level DingTalk gate: checked rows use DingTalk, unchecked rows fall back to the normal Feishu/wiki upload path for that run
 - if that checked row also has `DingTalk_target_node_url`, the worker uploads to that row-level target first; if it is blank, the `dingtalk_openapi` worker falls back to the Feishu control-row default target, while the legacy session path still falls back to `DINGTALK_DOCS_TARGET_NODE_URL`
...
1. `python build.py sync-data|dingtalk-control-config|process-build-queue|rst|html|word|pdf|all|review|check|sync-review|publish|diff-report|release-manifest|handoff|preview|fast|doctor`
...
- when `Document_link.Workflow_action = Publish` is consumed through the queue, keep `Document_link.Git_ref` pointed at the active review branch so the formal Publish DOCX and the latest publish HTML are both built from that same branch instead of drifting back to `main`.
...
- queue-driven `Workflow_action=Publish` additionally stages the formal DOCX under [`../reports/releases/<model>/<region>/<lang>/versions/<version>/`](../reports/releases) and mirrors the newest publish HTML under [`../reports/releases/<model>/<region>/<lang>/latest/html/`](../reports/releases) for Vercel
