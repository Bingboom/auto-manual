...
2. Data layer
   ...
   - during that refresh, any phase2 table that includes `Is_Latest` is repo-filtered down to `Is_Latest=TRUE`, so local phase2 snapshots do not keep older rows even if the Feishu view is wider
   ...
   - `python build.py listen-sync-data --config config.yaml --data-root data/phase2` is the local auto-sync listener for the phase2 source tables; it registers the current base for docs events through the local `lark-cli` login, listens on the app bot event stream, watches `spec_titles / spec_footnotes / spec_notes / symbols_blocks / spec_master`, and triggers a coalesced `sync-data` run whenever one of those tables changes in Feishu
   - after that local source-table sync succeeds, the listener also refreshes every existing local review bundle with `sync-review --sync-scope params --data-root data/phase2`, so the review layer picks up the newest phase2 data-driven RST automatically
  - `python build.py listen-phase2-events --config config.yaml --data-root data/phase2` is the recommended always-on listener when this same machine also needs `Document_link` immediate-build events; it keeps one shared event stream for both auto-sync and local build triggers
  - 这条本地 listener 走的是当前应用的 bot 身份；如果 bot 还不能对这张多维表调用 docs 变更订阅接口，启动时会直接报错，而不是表面看起来在监听但实际永远收不到事件
  - 但这不等于“本地都变成了 bot”：平时你手动跑 `python build.py sync-data`、`sync-review`、`process-build-queue`、`check`、`word`、`publish` 时，默认还是你本机 `lark-cli auth login` 的用户身份；只有这条长连接监听本身受 `lark-cli event +subscribe` 限制，必须走 bot 事件流
  - 远程 GitHub Actions 是另一条独立链路，它也用 bot，但那不应该反向改写“本地手动命令”的角色定义
  - 对 `Document_link` 来说，本地 listener 现在会在启动时先补扫一次已经 `是否触发文档构建=Y + 是否立即构建=true` 的记录；后面只要这张表有新增/修改事件，它也会重扫当前 immediate 队列，而不是只依赖 checkbox 差异 payload
   - the local listener entrypoints also share one single-instance lock now, so if one of those Feishu event listeners is already running, re-running another listener command exits cleanly instead of stealing the long-connection slot from the active process
  ...
  - `python build.py process-build-queue --config config.yaml --data-root data/phase2` is the optional Feishu task-table bridge: it reads `sync.phase2.document_link` rows where `是否触发文档构建 = Y`, writes `开始构建时间` as soon as one row is picked up, resolves the matching config family from `Document_Key + Lang`, builds the local Word file, uploads it to Feishu Drive, tries to move the uploaded file into the current wiki knowledge-base container, writes the local path into `Document directory`, writes the final link into `Document link`, writes a timestamped status into `构建结果`, and flips the trigger back to `已构建` on success
  - if that table also has `Doc_phase`, use `Draft` when the queue should build from the current `docs/_review` tree (`check --source review + word --source review`) and use `Publish` when it should run the full `build.py publish` path for the target; leave it blank only when you intentionally want the legacy `check + word` runtime path
  - once a target has entered review, those review-based build paths now auto-run `sync-review --sync-scope params` before `check/word/html/pdf/all/publish`, so preview and publish outputs refresh review-side parameter data before exporting
  - if your goal is simply "trigger a build and always use the latest phase2 data", prefer `process-build-queue`; it now refreshes `data/phase2` itself before it exports, so the long-connection listener is optional rather than required
  - `process-build-queue` also supports queue scoping: use `--doc-phase draft|publish` to consume one stage only, and add `--record-id <record_id>` when one workflow should build exactly one `Document_link` row
  - [`../scripts/listen_sync_data.ps1`](../scripts/listen_sync_data.ps1) is the Windows wrapper for source-table auto-sync; it restores the local Node/npm path plus the saved `FEISHU_PHASE2_*` user env vars, starts `listen-sync-data`, and writes logs into [`../.tmp/sync-data-listener/`](../.tmp/sync-data-listener)
  - [`../scripts/listen_phase2_events.ps1`](../scripts/listen_phase2_events.ps1) is the Windows wrapper for the shared phase2 event listener; it restores the same environment, starts `listen-phase2-events`, and writes logs into [`../.tmp/phase2-event-listener/`](../.tmp/phase2-event-listener)
  ...
  - `python build.py listen-build-queue --config config.yaml --data-root data/phase2` is the push-based immediate-build listener: after the Feishu app has the `drive.file.bitable_record_changed_v1` event enabled, it registers the table for docs events through the local `lark-cli` login and keeps the long connection on the app bot identity, then triggers `process-build-queue` immediately when `Document_link` rows are checked in `是否立即构建`
   ...
   - [`../.github/workflows/feishu-build-queue.yml`](../.github/workflows/feishu-build-queue.yml) is now the Publish-stage remote queue: it consumes only `Doc_phase=Publish` rows from `main`
   - [`../.github/workflows/feishu-draft-build-queue.yml`](../.github/workflows/feishu-draft-build-queue.yml) is the Draft-stage remote queue: dispatch it against the PR branch ref and it consumes only `Doc_phase=Draft` rows so the returned review link matches the PR branch content
   - set the repository variable `FEISHU_BUILD_QUEUE_PAUSED=true` when that remote queue should stay merged but temporarily stop polling; clear it or set it back to `false` when the poller may resume
   - [`../.github/workflows/phase2-word-preview.yml`](../.github/workflows/phase2-word-preview.yml) is the PR validation path for runtime docs: it syncs `data/phase2` from Feishu, runs `check + word` against the pull-request branch code for the explicit target matrix, and uploads the generated `.docx` files as workflow artifacts instead of consuming `Document_link`
  ...
  - when that move step fails, the queue now still preserves the latest uploaded Drive URL in `Document link`, keeps the latest local DOCX path in `Document directory`, clears `是否立即构建`, and records the wiki permission failure in `构建结果` so the table no longer keeps pointing to an older document link
  ...
  - if your goal is “source table changes should sync back to this local machine”, prefer `listen-sync-data` over a Feishu cloud workflow; the cloud workflow itself cannot directly execute a command on your local PC unless you separately expose a webhook/service from that machine
  - if this same machine also needs `Document_link` immediate-build events, prefer `listen-phase2-events` instead of running `listen-sync-data` and `listen-build-queue` separately; the current `lark-cli` only allows one live `event +subscribe` consumer per app
   - `page_registry.csv` and [`data/layout_params.csv`](../data/layout_params.csv) remain repo-maintained and are not overridden by `--data-root`
...
4. `product_name` is resolved from the active snapshot root, defaulting to [`data/phase1/Spec_Master.csv`](../data/phase1/Spec_Master.csv) and switching to [`data/phase2/Spec_Master.csv`](../data/phase2/Spec_Master.csv) when `--data-root data/phase2` is passed
...
- `python build.py sync-data --config config.yaml --data-root data/phase2` is the explicit local pull step for Feishu/Lark content; build commands never fetch online data implicitly.
- `python build.py sync-data --config config.yaml --data-root data/phase2 --dry-run` is the safest readiness probe for a new machine because it checks the local CLI/env prerequisites before attempting the real sync.
- `python build.py process-build-queue --config config.yaml --data-root data/phase2` is the explicit local consume-and-build step for the Feishu `Document_link` task table; it never runs implicitly from `sync-data`, `check`, or `publish`.
