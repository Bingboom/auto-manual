...
- `allowFrom` **先留空** = fail-closed(谁都不放行),步骤 4 拿到 staffId 再填。
...
1. 先重启网关让上面的空白名单生效:`openclaw gateway restart`
2. 在钉钉里**私聊该机器人**发一句(如 `hi`)。此时 fail-closed,机器人不会回复 —— 正常。
3. 读网关日志找发件人 staffId:
   ```bash
   openclaw channels logs --channel ddingtalk
   ```
   或在网关日志文件里搜 `收到消息`(日志路径见 `openclaw gateway status` 的 `File logs`),日志会带 `昵称(staffId)`。
4. 把取到的值填进 `openclaw.json` 的 `allowFrom`:
   ```json
