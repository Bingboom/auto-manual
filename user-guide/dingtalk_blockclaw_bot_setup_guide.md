...
## 让 BlockClaw 能查说明书目录(`发布文档管理`)

BlockClaw 回答"查 XX 的说明书链接"靠 `manual-index-query` / `product-manual-catalog` 技能读飞书 `发布文档管理` Base。这类读取**必须用稳定的企业应用(bot)token,别依赖 lark-cli 的 `user` 授权** —— user 的 OAuth scope 会过期 / 重新授权后变窄,典型表现是"昨天还好好的,今天突然报 `scope 不够` / `bitable:app` / `permission denied`"。

在网关加载的 `~/.auto_manual_feishu_phase2_env.sh` 里设:

```bash
export FEISHU_PHASE2_IDENTITY=bot
```

然后 `openclaw gateway restart`。前提:该机的飞书**应用**已开 `bitable:app:readonly` / `base:record:retrieve` 等读 scope,并对 `发布文档管理` Base 有访问权(`product-manual-catalog` 技能本就默认用 bot)。
...
| 重启网关后飞书短暂掉线 | 正常,几秒重连。 |
| 查说明书链接/目录报 `scope 不够` / `bitable:app` / `permission denied`(昨天还正常) | `manual-index-query` 默认走会过期的 lark-cli `user` 授权。在 `~/.auto_manual_feishu_phase2_env.sh` 设 `export FEISHU_PHASE2_IDENTITY=bot`(稳定的企业应用 token)、重启网关;应用需有 bitable 读 scope + `发布文档管理` Base 访问权。 |
